    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("""
                ...
                -- without chat_id. CREATE TABLE IF NOT EXISTS does not migrate
                -- an existing table, so add the column before creating its index.
                ...
                CREATE TABLE IF NOT EXISTS quiz_schedules (
                    ...
                    group_card_message_id BIGINT,
                    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
                    started_at TIMESTAMP WITH TIME ZONE,
                    completed_at TIMESTAMP WITH TIME ZONE,
                    cancelled_at TIMESTAMP WITH TIME ZONE,
                    CONSTRAINT quiz_schedules_status_check
                        CHECK (status IN ('scheduled','running','completed','cancelled','failed')),
                    CONSTRAINT quiz_schedules_timer_check
                        CHECK (timer IN (10,15,20,30,45,60))
                );

                CREATE INDEX IF NOT EXISTS idx_quiz_schedules_chat_time
                ON quiz_schedules(chat_id, scheduled_at);

                CREATE INDEX IF NOT EXISTS idx_quiz_schedules_status_time
...
def parse_india_datetime(date_text: str, time_text: str):
    """Parse a user supplied date/time as Asia/Kolkata (IST)."""
    date_value = None
    time_value = None

    for fmt in ("%d/%m/%Y", "%d-%m-%Y", "%d %m %Y", "%Y-%m-%d"):
        try:
            date_value = datetime.strptime(date_text.strip(), fmt).date()
            break
        except ValueError:
            pass
    ...
    for fmt in ("%I:%M %p", "%I %p", "%H:%M"):
        try:
            time_value = datetime.strptime(time_text.strip().upper(), fmt).time()
            break
        except ValueError:
            pass

    if time_value is None:
        raise ValueError("Invalid time. Use HH:MM AM/PM.")

    return datetime.combine(date_value, time_value, tzinfo=SCHEDULE_TZ)


def schedule_display_time(value) -> str:
...
async def schedule_conflict_exists(chat_id: int, scheduled_at: datetime, exclude_id: int = None) -> bool:
    if not db_pool:
        return True
...
async def create_quiz_schedule(data: dict):
    """Create a one-time schedule with an atomic final conflict check."""
    if not db_pool:
        raise RuntimeError("Database is not ready.")

    async with db_pool.acquire() as conn:
        async with conn.transaction():
            # Serialize schedule creation for the same target chat so two admins
            # cannot create conflicting schedules at the same instant.
            await conn.execute(
                "SELECT pg_advisory_xact_lock(hashtext($1))",
                f"quiz-schedule:{data['chat_id']}"
            )
            ...
            row = await conn.fetchrow(
                ...
                (quiz_id, chat_id, chat_username, chat_title, scheduled_at,
                 created_by, creator_name, qshuffle, oshuffle, explanation, timer)
...
async def update_schedule_card(bot, schedule_id: int, text: str, keyboard=None):
    row = await get_schedule(schedule_id)
    if not row or not row["group_card_message_id"]:
        return
    try:
        await bot.edit_message_text(
            chat_id=int(row["chat_id"]),
            message_id=int(row["group_card_message_id"]),
            text=text,
            parse_mode="HTML",
...
def schedule_card_text(row, cancelled=False):
    data = dict(row)
    title = safe_html(data.get("title") or "Quiz")
    scheduled = schedule_display_time(data["scheduled_at"])
    creator = safe_html(data.get("creator_name") or str(data["created_by"]))
    timer = int(data["timer"])
    if cancelled:
        return (
            f"🧠 <b>{title}</b>\n\n"
            "━━━━━━━━━━━━━━\n\n"
            "❌ <b>Schedule Cancelled</b>\n\n"
            f"📅 {scheduled}\n"
            f"👤 Scheduled by: {creator}"
        )
    return (
        f"🧠 <b>{title}</b>\n\n"
        "━━━━━━━━━━━━━━\n\n"
        f"📅 <b>Date:</b> {scheduled.split(',')[0]}\n"
        f"🕐 <b>Time:</b> {scheduled.split(', ',1)[1]} IST\n"
        f"⏱️ <b>Timer:</b> {timer} Seconds\n"
        f"👤 <b>Scheduled by:</b> {creator}\n\n"
        "🔔 Quiz will automatically start at the scheduled time."
    )


def build_schedule_cancel_keyboard(schedule_id: int):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("❌ Cancel Schedule", callback_data=f"schedule_cancel_{schedule_id}")]
    ])


async def mark_schedule_card_message(bot, schedule_id: int, message_id: int):
    if not db_pool:
        return
    async with db_pool.acquire() as conn:
        await conn.execute(
            "UPDATE quiz_schedules SET group_card_message_id=$1 WHERE id=$2",
            message_id, schedule_id
        )


async def cancel_quiz_schedule(schedule_id: int, user_id: int, bot) -> str:
    row = await get_schedule(schedule_id)
    if not row:
        return "missing"
    ...
    try:
        chat = await bot.get_chat(int(row["chat_id"]))
    ...
    await update_schedule_card(
        bot,
        schedule_id,
        schedule_card_text(changed, cancelled=True),
        None
    )
    return "ok"
...
async def load_quiz_for_schedule(quiz_id: int):
    ...
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT id, owner_id, title, questions, short_code FROM quizzes WHERE id=$1",
            quiz_id
        )
    if not row:
        return None
...
async def scheduler_start_schedule(application: Application, row):
    schedule_id = int(row["id"])
    chat_id = int(row["chat_id"])

    # Never start two quizzes in the same group simultaneously.
    if await redis_client.get(f"quiz_active:{chat_id}"):
        return False

    # After a long-running quiz, preserve the agreed five-minute cooldown.
    last_finished = await redis_client.get(f"quiz_last_finished:{chat_id}")
    if last_finished:
        try:
            remaining = SCHEDULE_GAP_SECONDS - (time.time() - float(last_finished))
            if remaining > 0:
                return False
        except (TypeError, ValueError):
            pass

    quiz_loaded = await load_quiz_for_schedule(int(row["quiz_id"]))
    if not quiz_loaded:
        async with db_pool.acquire() as conn:
            await conn.execute(
                "UPDATE quiz_schedules SET status='failed' WHERE id=$1 AND status='scheduled'",
                schedule_id
            )
        return False

    # Atomically claim this schedule before starting it.
    async with db_pool.acquire() as conn:
    ...
    schedule_context = SimpleNamespace(
        ...
        user_data={
            ...
            "short_code": row["chat_username"],
            "schedule_id": schedule_id
        }
    )

    try:
        started = await begin_quiz(chat_id, schedule_context, suppress_start_message=True)
        if not started:
            async with db_pool.acquire() as conn:
                await conn.execute(
                    "UPDATE quiz_schedules SET status='scheduled', started_at=NULL WHERE id=$1 AND status='running'",
                    schedule_id
                )
            return False
    except Exception:
        logger.exception("Scheduled quiz failed to start. schedule=%s", schedule_id)
        async with db_pool.acquire() as conn:
            await conn.execute(
                "UPDATE quiz_schedules SET status='failed' WHERE id=$1 AND status='running'",
                schedule_id
            )
        return False
...
async def scheduler_loop(application: Application):
    logger.info("⏰ Quiz scheduler started (Asia/Kolkata).")
    while True:
        try:
            now = datetime.now(SCHEDULE_TZ)
            reminder_cutoff = now + timedelta(minutes=5)
            ...
            if db_pool:
                    ...
                    try:
                        ...
                        async with db_pool.acquire() as conn:
                            await conn.execute(
                                ...
                                WHERE id=$1 AND status='scheduled' AND reminder_sent=FALSE
                                """,
                                row["id"]
                            )
                    except TelegramError:
                        logger.exception("Could not send schedule reminder: %s", row["id"])
                ...
                for row in due_rows:
                    await scheduler_start_schedule(application, row)
...
def build_schedule_settings_keyboard(data: dict) -> InlineKeyboardMarkup:
    q_shuffle = "ON" if data.get("schedule_qshuffle", data.get("qshuffle", False)) else "OFF"
    o_shuffle = "ON" if data.get("schedule_oshuffle", data.get("oshuffle", False)) else "OFF"
    explanation = "ON" if data.get("schedule_explanation", data.get("explanation", True)) else "OFF"
    timer = data.get("schedule_timer", data.get("timer", 20))
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"🔀 Questions: {q_shuffle}", callback_data="schedule_toggle_qshuffle"),
         InlineKeyboardButton(f"🔀 Options: {o_shuffle}", callback_data="schedule_toggle_oshuffle")],
        [InlineKeyboardButton(f"💡 Explanation: {explanation}", callback_data="schedule_toggle_exp")],
        [InlineKeyboardButton(f"{'🔘 ' if timer == 10 else ''}10s", callback_data="schedule_timer_10"),
         InlineKeyboardButton(f"{'🔘 ' if timer == 15 else ''}15s", callback_data="schedule_timer_15"),
         InlineKeyboardButton(f"{'🔘 ' if timer == 20 else ''}20s", callback_data="schedule_timer_20")],
        [InlineKeyboardButton(f"{'🔘 ' if timer == 30 else ''}30s", callback_data="schedule_timer_30"),
         InlineKeyboardButton(f"{'🔘 ' if timer == 45 else ''}45s", callback_data="schedule_timer_45"),
         InlineKeyboardButton(f"{'🔘 ' if timer == 60 else ''}60s", callback_data="schedule_timer_60")],
        [InlineKeyboardButton("✅ Confirm Schedule", callback_data="schedule_confirm"),
         InlineKeyboardButton("❌ Cancel", callback_data="schedule_abort")]
    ])
...
async def button_handler(
    ...
    if data == "schedule_quiz":
        if query.message.chat.type != "private":
            await query.answer(
                "⏰ Please schedule from private chat.",
                show_alert=True
            )
            return

        if not context.user_data.get("quiz_id") or not context.user_data.get("questions"):
            await query.answer("❌ Load a saved quiz first.", show_alert=True)
            return
        ...
        context.user_data["schedule_target_chat"] = None
        context.user_data["schedule_target_raw"] = None
        context.user_data["schedule_at"] = None
        context.user_data["schedule_qshuffle"] = bool(context.user_data.get("qshuffle", False))
        context.user_data["schedule_oshuffle"] = bool(context.user_data.get("oshuffle", False))
        context.user_data["schedule_explanation"] = bool(context.user_data.get("explanation", True))
        context.user_data["schedule_timer"] = int(context.user_data.get("timer", 20))

        await query.answer()
        await query.message.reply_text(
            "⏰ <b>Schedule Quiz</b>\n\n"
            "Send the <b>public group username or public Telegram link</b>.\n\n"
            "Example: <code>@myquizgroup</code>\n"
            "or <code>https://t.me/myquizgroup</code>\n\n"
            "🆔 Chat ID is not required.\n"
            "🇮🇳 Timezone: India (IST)",
            parse_mode="HTML"
        )
        return
    ...
    if data == "schedule_toggle_qshuffle":
        context.user_data["schedule_qshuffle"] = not context.user_data.get("schedule_qshuffle", False)
        await query.answer()
        await query.edit_message_reply_markup(reply_markup=build_schedule_settings_keyboard(context.user_data))
        return

    if data == "schedule_toggle_oshuffle":
        context.user_data["schedule_oshuffle"] = not context.user_data.get("schedule_oshuffle", False)
        await query.answer()
        await query.edit_message_reply_markup(reply_markup=build_schedule_settings_keyboard(context.user_data))
        return

    if data == "schedule_toggle_exp":
        context.user_data["schedule_explanation"] = not context.user_data.get("schedule_explanation", True)
        await query.answer()
        await query.edit_message_reply_markup(reply_markup=build_schedule_settings_keyboard(context.user_data))
        return

    if data.startswith("schedule_timer_"):
        timer_val = int(data.rsplit("_", 1)[1])
        context.user_data["schedule_timer"] = timer_val
        await query.answer(f"Timer set to {timer_val}s")
        await query.edit_message_reply_markup(reply_markup=build_schedule_settings_keyboard(context.user_data))
        return

    if data == "schedule_abort":
        for key in [
            "waiting_schedule_target", "waiting_schedule_date", "waiting_schedule_time",
            "schedule_target_chat", "schedule_target_raw", "schedule_at", "schedule_date_text",
            "schedule_qshuffle", "schedule_oshuffle", "schedule_explanation", "schedule_timer"
        ]:
            context.user_data.pop(key, None)
        await query.answer("❌ Scheduling cancelled.")
        await query.edit_message_text("❌ <b>Schedule creation cancelled.</b>", parse_mode="HTML")
        return

    # Confirm a new schedule
    if data == "schedule_confirm":
        if query.message.chat.type != "private":
            await query.answer("❌ Use private chat.", show_alert=True)
            return

        target = context.user_data.get("schedule_target_chat")
        schedule_at = context.user_data.get("schedule_at")
        quiz_id = context.user_data.get("quiz_id")
        user_id = query.from_user.id

        if not target or not schedule_at or not quiz_id:
            await query.answer("❌ Schedule information is incomplete.", show_alert=True)
            return

        try:
            # Final user and bot permission verification immediately before save.
            target = await context.bot.get_chat(target.id)
            if not await is_target_chat_admin_or_owner(target, user_id):
                await query.answer("❌ You are no longer a Group Admin/Owner.", show_alert=True)
                return

            bot_info = await context.bot.get_me()
            bot_member = await context.bot.get_chat_member(target.id, bot_info.id)
            if bot_member.status not in ["creator", "administrator"]:
                await query.answer("❌ Bot is no longer an Administrator in this group.", show_alert=True)
                return
        except TelegramError:
            await query.answer("❌ Group permissions could not be verified.", show_alert=True)
            return

        if schedule_at <= datetime.now(SCHEDULE_TZ):
            await query.answer("⚠️ Please choose a future date and time.", show_alert=True)
            return

        data_to_save = {
            "quiz_id": int(quiz_id),
            "chat_id": int(target.id),
            "chat_username": context.user_data.get("schedule_target_raw") or (f"@{target.username}" if target.username else str(target.id)),
            "chat_title": target.title or "Group",
            "scheduled_at": schedule_at,
            "created_by": int(user_id),
            "creator_name": query.from_user.full_name,
            "qshuffle": bool(context.user_data.get("schedule_qshuffle", context.user_data.get("qshuffle", False))),
            "oshuffle": bool(context.user_data.get("schedule_oshuffle", context.user_data.get("oshuffle", False))),
            "explanation": bool(context.user_data.get("schedule_explanation", context.user_data.get("explanation", True))),
            "timer": int(context.user_data.get("schedule_timer", context.user_data.get("timer", 20)))
        }

        schedule_id, error = await create_quiz_schedule(data_to_save)
        if error == "conflict":
            await query.answer("⚠️ Schedule conflict: keep at least 5 minutes between schedules in this group.", show_alert=True)
            return
        if not schedule_id:
            await query.answer("❌ Could not create the schedule.", show_alert=True)
            return

        row = await get_schedule(schedule_id)
        card = schedule_card_text(row)
        try:
            group_message = await context.bot.send_message(
                int(target.id),
                card,
                parse_mode="HTML",
                reply_markup=build_schedule_cancel_keyboard(schedule_id)
            )
            await mark_schedule_card_message(context.bot, schedule_id, group_message.message_id)
        except TelegramError:
            async with db_pool.acquire() as conn:
                await conn.execute(
                    "UPDATE quiz_schedules SET status='failed' WHERE id=$1 AND status='scheduled'",
            ...
            await query.message.reply_text(
                "❌ The schedule was not activated because the bot could not send the group schedule card."
            )
            return

        await query.answer("✅ Schedule created.")
        await query.message.reply_text(
            f"🧠 <b>{safe_html(row['title'])}</b>\n\n"
            "✅ <b>Quiz Scheduled Successfully</b>\n\n"
            f"👥 Group: {safe_html(data_to_save['chat_username'])}\n"
            f"📅 Date: {schedule_at.astimezone(SCHEDULE_TZ).strftime('%d %B %Y')}\n"
            f"🕐 Time: {schedule_at.astimezone(SCHEDULE_TZ).strftime('%I:%M %p')} IST\n"
            f"⏱️ Timer: {data_to_save['timer']} Seconds\n\n"
        ...
        for key in [
            "waiting_schedule_target", "waiting_schedule_date", "waiting_schedule_time",
            "schedule_target_chat", "schedule_target_raw", "schedule_at",
            "schedule_qshuffle", "schedule_oshuffle", "schedule_explanation", "schedule_timer"
        ]:
            context.user_data.pop(key, None)
        return

    if data == "schedule_cancel_confirm":
        await query.answer("Use the group schedule card to cancel this schedule.", show_alert=True)
        return

    if data.startswith("schedule_cancel_yes_"):
        try:
        ...
        if query.message.chat.type not in ["group", "supergroup"]:
            await query.answer("❌ This control is only available in the scheduled group.", show_alert=True)
            return
        result = await cancel_quiz_schedule(schedule_id, query.from_user.id, context.bot)
        if result == "ok":
            await query.answer("✅ Schedule cancelled.")
            try:
                await query.message.edit_text("❌ <b>Schedule Cancelled</b>", parse_mode="HTML")
            except TelegramError:
                pass
        elif result == "denied":
            await query.answer("❌ Only Group Admins and Owner can cancel this schedule.", show_alert=True)
        else:
            await query.answer("ℹ️ This schedule is no longer active.", show_alert=True)
        return

    if data.startswith("schedule_cancel_back_"):
        await query.answer()
        try:
    ...
    if data.startswith("schedule_cancel_"):
        ...
        row = await get_schedule(schedule_id)
        if not row or int(row["chat_id"]) != int(query.message.chat.id):
            await query.answer("❌ Schedule not found.", show_alert=True)
            return

        if not await is_target_chat_admin_or_owner(query.message.chat, query.from_user.id):
            await query.answer("❌ Only Group Admins and Owner can cancel this schedule.", show_alert=True)
            return
        ...
        confirm_markup = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Yes, Cancel", callback_data=f"schedule_cancel_yes_{schedule_id}"),
             InlineKeyboardButton("↩️ Back", callback_data=f"schedule_cancel_back_{schedule_id}")]
        ])
        await query.answer()
        await query.message.reply_text(
            f"⚠️ <b>Cancel this scheduled quiz?</b>\n\n"
            f"🕐 {schedule_display_time(row['scheduled_at'])}",
            parse_mode="HTML",
...
async def receive_text(
    ...
    # Schedule target/date/time inputs are handled before other private states.
    if context.user_data.get("waiting_schedule_target"):
        raw_target = text
        # Scheduling intentionally accepts only public group usernames/links.
        parsed = normalize_target_input(raw_target)
        if not isinstance(parsed, str) or not parsed.startswith("@"):
            await update.message.reply_text(
                "❌ Please send a public group username or public Telegram link.\n\n"
                "Example: <code>@myquizgroup</code> or <code>https://t.me/myquizgroup</code>",
                parse_mode="HTML"
            )
            return

        chat, error = await resolve_target_chat(context.bot, raw_target)
        if error:
            await update.message.reply_text(error, parse_mode="HTML")
            return
        if chat.type not in ["group", "supergroup"]:
            await update.message.reply_text("❌ Scheduling is available for groups/supergroups only.")
            return
        if not await is_target_chat_admin_or_owner(chat, update.effective_user.id):
            await update.message.reply_text(
                "❌ <b>Permission Denied</b>\n\nOnly the Group Admin or Owner can schedule a quiz in this group.",
                parse_mode="HTML"
            )
            return

        context.user_data["schedule_target_chat"] = chat
        context.user_data["schedule_target_raw"] = raw_target
    ...
    if context.user_data.get("waiting_schedule_date"):
        try:
            ...
            await update.message.reply_text("❌ Invalid date. Use <code>DD/MM/YYYY</code>.", parse_mode="HTML")
            return
        context.user_data["schedule_date_text"] = text
        context.user_data["waiting_schedule_date"] = False
        context.user_data["waiting_schedule_time"] = True
        await update.message.reply_text(
            "🕐 <b>Enter Quiz Start Time</b>\n\nExample: <code>08:00 PM</code>\n\n🇮🇳 India (IST)",
            parse_mode="HTML"
        )
        return
    ...
    if context.user_data.get("waiting_schedule_time"):
        try:
            schedule_at = parse_india_datetime(
                context.user_data.get("schedule_date_text", ""),
                text
            )
        ...
        if schedule_at <= datetime.now(SCHEDULE_TZ):
            await update.message.reply_text("⚠️ Please choose a future date and time.")
            return

        target = context.user_data.get("schedule_target_chat")
        if not target:
            await update.message.reply_text("❌ Group information is missing. Start scheduling again.")
            return

        if await schedule_conflict_exists(target.id, schedule_at):
            await update.message.reply_text(
                "⚠️ <b>Schedule Time Conflict</b>\n\n"
                "Another quiz is already scheduled in this group within the required 5-minute gap.\n\n"
        ...
        context.user_data["waiting_schedule_time"] = False
        context.user_data["schedule_date_text"] = None

        title = context.user_data.get("quiz_title", "Quiz")
        await update.message.reply_text(
            f"🧠 <b>{safe_html(title)}</b>\n\n"
            "⚙️ <b>Schedule Settings</b>\n\n"
            "Choose the quiz settings below. These settings apply to this schedule only.",
            parse_mode="HTML",
            reply_markup=build_schedule_settings_keyboard(context.user_data)
        )
        return
...
async def begin_quiz(
    chat_id: int,
    context: ContextTypes.DEFAULT_TYPE,
    suppress_start_message: bool = False
):
    ...
    try:
        ...
        await redis_client.hset(
            ...
            mapping={
                ...
                "schedule_id": str(context.user_data.get("schedule_id", ""))
            }
        )
...
async def finish_quiz(
    ...
    try:
        ...
        schedule_id_raw = quiz_info.get("schedule_id")
        if schedule_id_raw:
            try:
                ...
                logger.warning("Invalid schedule_id in quiz state: %r", schedule_id_raw)

        await redis_client.set(
            f"quiz_last_finished:{chat_key}",
            str(time.time()),
            ex=3600
        )
...
async def post_init(
    ...
    # Persistent one-time quiz scheduler. Schedules are stored in PostgreSQL
    # and recovered automatically after a Render restart/redeploy.
