        ...
        async with db_pool.acquire() as conn:
            await conn.execute("""
                ...
                CREATE TABLE IF NOT EXISTS group_schedules (
                    ...
                    execution_state VARCHAR(16) NOT NULL DEFAULT 'pending',
                ...
                ALTER TABLE group_schedules
                    ...
                    ADD COLUMN IF NOT EXISTS execution_state VARCHAR(16) NOT NULL DEFAULT 'pending',
                ...
                ON group_schedules(status, scheduled_at);

                CREATE INDEX IF NOT EXISTS idx_group_schedules_group
                ON group_schedules(target_group_id, scheduled_at);
            """)
...
def parse_schedule_datetime(raw: str) -> Optional[datetime]:
    """Parse a user supplied schedule time as Asia/Kolkata aware datetime."""
    value = raw.strip()
    now = datetime.now(IST)

    # Useful for quick tests/admins: +2m, +5 minutes, +1h 30m.
    relative = re.fullmatch(
        r"\+\s*(?:(\d+)\s*(?:h|hr|hrs|hour|hours))?\s*"
...
def build_schedule_keyboard(schedule_id: int, live: bool) -> InlineKeyboardMarkup:
    if live:
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(
            start_text,
            callback_data=f"schedule_start_{schedule_id}"
        )],
        [InlineKeyboardButton(
            share_text,
            callback_data=f"schedule_share_{schedule_id}"
        )],
        [InlineKeyboardButton(
            "🗑 Cancel Schedule",
            callback_data=f"schedule_cancel_{schedule_id}"
        )],
    ])


def build_schedule_card(row, live: bool = False) -> str:
    scheduled = row["scheduled_at"]
    if live:
        return (
            f"📅 <b>Scheduled for:</b>\n"
            f"{format_schedule_time(scheduled)}\n\n"
            "🟢 <b>LIVE</b>"
        )
    return (
        f"📅 <b>Scheduled for:</b>\n"
...
async def create_group_schedule_async(
    quiz_id: int,
    owner_id: int,
    target_group_id: int,
    ...
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            INSERT INTO group_schedules
                (quiz_id, owner_id, target_group_id, scheduled_at,
                 timezone, status, settings)
            VALUES ($1, $2, $3, $4, 'Asia/Kolkata', 'scheduled', $5::jsonb)
            RETURNING id
            """,
            quiz_id,
            owner_id,
            target_group_id,
            scheduled_at.astimezone(IST),
            json.dumps(settings, ensure_ascii=False),
        )
        return int(row["id"])


async def get_group_schedule_async(schedule_id: int):
    if not db_pool:
        return None
    async with db_pool.acquire() as conn:
        return await conn.fetchrow(
            """
            SELECT s.*, q.title, q.questions, q.short_code
...
async def set_schedule_card_message_async(schedule_id: int, message_id: int) -> None:
    if not db_pool:
        return
    async with db_pool.acquire() as conn:
        await conn.execute(
            ...
            SET card_message_id=$2, updated_at=CURRENT_TIMESTAMP
            WHERE id=$1
            """,
            schedule_id,
            message_id,
...
async def cancel_group_schedule_async(schedule_id: int, user_id: int, chat) -> str:
    if not db_pool:
        return "missing"
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            SELECT id, owner_id, target_group_id, status
        ...
        if not await is_target_chat_admin_or_owner(chat, user_id) and str(user_id) != str(row["owner_id"]):
            return "forbidden"
        await conn.execute(
            """
            UPDATE group_schedules
            SET status='cancelled', execution_state='cancelled', updated_at=CURRENT_TIMESTAMP
...
async def claim_schedule_execution(schedule_id: int):
    """Atomically move scheduled -> live/starting; only one worker can win."""
    if not db_pool:
        return None
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            row = await conn.fetchrow(
                """
                UPDATE group_schedules
                SET status='live', execution_state='starting',
            ...
            # Recovery after a process crash between the atomic claim and start.
            row = await conn.fetchrow(
                """
                UPDATE group_schedules
                SET execution_state='starting',
                    execution_claimed_at=CURRENT_TIMESTAMP,
                    updated_at=CURRENT_TIMESTAMP
                WHERE id=$1
                  AND status='live'
                  AND execution_state IN ('starting','pending')
                  AND (execution_claimed_at IS NULL OR execution_claimed_at < CURRENT_TIMESTAMP - INTERVAL '5 seconds')
                RETURNING *
                """,
                schedule_id,
            )
...
async def mark_schedule_started(schedule_id: int) -> None:
    if not db_pool:
        return
    async with db_pool.acquire() as conn:
        await conn.execute(
            """
            UPDATE group_schedules
            SET execution_state='started', execution_claimed_at=NULL,
                updated_at=CURRENT_TIMESTAMP
...
async def mark_schedule_start_failed(schedule_id: int) -> None:
    if not db_pool:
        return
    async with db_pool.acquire() as conn:
        await conn.execute(
            """
            UPDATE group_schedules
            SET execution_state='pending', execution_claimed_at=NULL,
                updated_at=CURRENT_TIMESTAMP
            WHERE id=$1 AND status='live' AND execution_state='starting'
            """,
            schedule_id,
        )


async def claim_schedule_reminder(schedule_id: int) -> bool:
    if not db_pool:
        return False
    async with db_pool.acquire() as conn:
        result = await conn.execute(
            ...
            WHERE id=$1 AND status='scheduled' AND reminder_sent=FALSE
              AND scheduled_at > CURRENT_TIMESTAMP
              AND scheduled_at <= CURRENT_TIMESTAMP + INTERVAL '5 minutes'
            """,
            schedule_id,
        )
...
async def list_due_group_schedules():
    if not db_pool:
        return []
    async with db_pool.acquire() as conn:
        return await conn.fetch(
            ...
            WHERE (
                s.status='scheduled'
                AND s.scheduled_at <= CURRENT_TIMESTAMP + INTERVAL '5 minutes'
            )
            OR (
                s.status='live' AND s.execution_state IN ('starting','pending')
                AND (s.execution_claimed_at IS NULL OR s.execution_claimed_at < CURRENT_TIMESTAMP - INTERVAL '5 seconds')
            )
...
async def safe_edit_schedule_card(bot, row, live: bool) -> bool:
    message_id = row["card_message_id"]
    if not message_id:
        return False
    text = build_schedule_card(row, live=live)
    markup = build_schedule_keyboard(int(row["id"]), live=live)
    for _ in range(3):
        try:
            await bot.edit_message_text(
                chat_id=int(row["target_group_id"]),
            ...
            logger.warning("Schedule card edit failed id=%s: %s", row["id"], e)
            return False
    return False


async def send_group_schedule_reminder(bot, row):
    for _ in range(3):
        try:
            await bot.send_message(
                int(row["target_group_id"]),
                "⏰ <b>5-minute reminder</b>\n"
                f"Scheduled quiz starts at <b>{format_schedule_time(row['scheduled_at'])}</b>.",
                parse_mode="HTML",
            )
            return
        except RetryAfter as e:
            ...
            logger.warning("5-minute reminder failed id=%s: %s", row["id"], e)
            return


async def send_schedule_card(bot, row, live: bool = False):
    markup = build_schedule_keyboard(int(row["id"]), live=live)
    text = build_schedule_card(row, live=live)
    for _ in range(3):
        try:
            ...
            logger.warning("Schedule card send failed id=%s: %s", row["id"], e)
            return None
    return None


async def execute_group_schedule(application: Application, schedule_id: int):
    row = await claim_schedule_execution(schedule_id)
    if not row:
        return

    bot = application.bot
    try:
        # If creation crashed before card_message_id was stored, rebuild the SAME card now.
        if not row["card_message_id"]:
            msg = await send_schedule_card(bot, row, live=True)
            if msg:
                await set_schedule_card_message_async(schedule_id, msg.message_id)
                row = dict(row)
                row["card_message_id"] = msg.message_id
        else:
            await safe_edit_schedule_card(bot, row, live=True)
        ...
        schedule_context = SimpleNamespace(bot=bot, user_data={})
        ...
        if started:
            await mark_schedule_started(schedule_id)
        else:
            # If another live quiz is currently occupying the group, keep the schedule LIVE
            # and retry later rather than claiming a duplicate automatic start.
            await mark_schedule_start_failed(schedule_id)
    except Exception:
        logger.exception("Scheduled group quiz execution failed id=%s", schedule_id)
        await mark_schedule_start_failed(schedule_id)


async def group_schedule_worker(application: Application):
    """Persistent DB-backed scheduler. Restart recovery comes from PostgreSQL, not memory."""
    while True:
        try:
            rows = await list_due_group_schedules()
            now = datetime.now(IST)
            for row in rows:
                schedule_id = int(row["id"])
                scheduled_at = row["scheduled_at"].astimezone(IST)
                if row["status"] == "scheduled":
                    if scheduled_at > now:
                        if scheduled_at <= now + timedelta(minutes=5):
                            if await claim_schedule_reminder(schedule_id):
                                # The claim prevents duplicates; the retry task cannot block other schedules.
                                track_task(asyncio.create_task(
                                    send_group_schedule_reminder(application.bot, row)
                                ))
                        continue
                    track_task(asyncio.create_task(execute_group_schedule(application, schedule_id)))
                elif row["status"] == "live" and row["execution_state"] == "starting":
                    track_task(asyncio.create_task(execute_group_schedule(application, schedule_id)))
        except asyncio.CancelledError:
            raise
        except Exception:
            logger.exception("Group schedule worker iteration failed; continuing.")
...
async def button_handler(
    ...
    # GROUP SCHEDULE ONLY. No Channel Schedule callbacks exist here.
    if data == "schedule_group":
        if query.message.chat.type != "private":
            await query.answer("📅 Schedule Group from private chat.", show_alert=True)
            return

        quiz_id = int(context.user_data.get("quiz_id", 0) or 0)
        owner_id = int(context.user_data.get("owner_id", 0) or 0)
        if not quiz_id or owner_id != query.from_user.id:
            await query.answer("❌ Only the quiz owner can create this schedule.", show_alert=True)
            return
        ...
        await query.answer()
        await query.message.reply_text(
            "📅 <b>Schedule Group Quiz</b>\n\n"
            "Send the target <b>Group/Supergroup</b> username or Chat ID.\n"
            "Example: <code>@mygroup</code> or <code>-1001234567890</code>\n\n"
            "⚠️ Channel scheduling is not supported.",
            parse_mode="HTML"
        )
        return

    if data.startswith("schedule_start_"):
        try:
            schedule_id = int(data.rsplit("_", 1)[1])
        except ValueError:
            await query.answer("❌ Invalid schedule.", show_alert=True)
            return
        row = await get_group_schedule_async(schedule_id)
        if not row:
            await query.answer("❌ Schedule not found.", show_alert=True)
            return
        if int(row["target_group_id"]) != int(query.message.chat_id):
            await query.answer("❌ This schedule belongs to another group.", show_alert=True)
            return
        if row["status"] != "live":
            await query.answer("🔒 This scheduled quiz is still locked.", show_alert=True)
            return
        ...
        await query.answer()
        started = await begin_quiz(query.message.chat_id, SimpleNamespace(bot=context.bot, user_data={}), quiz_data=quiz_data)
        if started:
            await mark_schedule_started(schedule_id)
        return

    if data.startswith("schedule_share_"):
        try:
            schedule_id = int(data.rsplit("_", 1)[1])
        except ValueError:
            await query.answer("❌ Invalid schedule.", show_alert=True)
            return
        row = await get_group_schedule_async(schedule_id)
        if not row or int(row["target_group_id"]) != int(query.message.chat_id):
            await query.answer("❌ Schedule not found.", show_alert=True)
            return
        if row["status"] != "live":
            await query.answer("🔒 Sharing is locked until the scheduled time.", show_alert=True)
            return
        share_markup = InlineKeyboardMarkup([
            [InlineKeyboardButton(
                "📱 Share quiz",
                switch_inline_query=f"quiz:{row['short_code']}"
            )]
        ])
        await query.answer("📱 Share options sent.")
        await query.message.reply_text(
            "📱 <b>Share this quiz</b>",
            parse_mode="HTML",
    ...
    if data.startswith("schedule_cancel_"):
        ...
        if int(row["target_group_id"]) != int(query.message.chat_id):
            ...
            return
        if row["status"] != "scheduled":
            await query.answer("❌ Only a scheduled (not LIVE) schedule can be cancelled.", show_alert=True)
            return
        if not await is_target_chat_admin_or_owner(query.message.chat, query.from_user.id) and str(query.from_user.id) != str(row["owner_id"]):
            await query.answer("⚠️ Only the group admin/owner can cancel this schedule.", show_alert=True)
            return
        result = await cancel_group_schedule_async(schedule_id, query.from_user.id, query.message.chat)
        if result == "cancelled":
            await query.answer("🗑 Schedule cancelled.")
            try:
                await query.edit_message_text(
                    build_schedule_card(row, live=False) + "\n\n🔴 <b>CANCELLED</b>",
                    parse_mode="HTML",
            ...
            await query.answer("❌ Schedule is no longer pending.", show_alert=True)
...
async def receive_text(
    ...
    # GROUP SCHEDULE: target group input.
    if context.user_data.get("waiting_schedule_group"):
        quiz_id = int(context.user_data.get("quiz_id", 0) or 0)
        if not quiz_id:
            context.user_data["waiting_schedule_group"] = False
            await update.message.reply_text("❌ Quiz context is missing. Load the quiz again.")
            return

        chat, error_message = await resolve_target_chat(context.bot, text)
        if error_message:
            await update.message.reply_text(error_message, parse_mode="HTML")
            return
        if chat.type not in ["group", "supergroup"]:
            await update.message.reply_text("❌ Schedule Group supports only Group/Supergroup. Channel scheduling is disabled.")
            return
        if not await is_target_chat_admin_or_owner(chat, update.effective_user.id) and str(update.effective_user.id) != str(context.user_data.get("owner_id")):
            await update.message.reply_text("❌ Only the target Group Admin/Owner or Quiz Owner can schedule this quiz.")
            return

        context.user_data["schedule_target_group_id"] = int(chat.id)
        context.user_data["waiting_schedule_group"] = False
        context.user_data["waiting_schedule_time"] = True
        await update.message.reply_text(
            "⏰ <b>Send scheduled time in IST (Asia/Kolkata)</b>\n\n"
            "Examples:\n"
            "<code>17-08-2026 10:30</code>\n"
            "<code>17-08-2026 10:30 AM</code>\n"
            "<code>+5m</code> (five minutes from now)\n\n"
            "The time is stored as a timezone-aware IST datetime.",
            parse_mode="HTML"
        )
        return

    # GROUP SCHEDULE: datetime input and persistent DB insert.
    if context.user_data.get("waiting_schedule_time"):
        scheduled_at = parse_schedule_datetime(text)
        if not scheduled_at:
            await update.message.reply_text(
                "❌ Invalid date/time. Use <code>17-08-2026 10:30</code>, <code>17-08-2026 10:30 AM</code> or <code>+5m</code>.",
                parse_mode="HTML"
            )
            return
        ...
        quiz_id = int(context.user_data.get("quiz_id", 0) or 0)
        owner_id = int(context.user_data.get("owner_id", 0) or 0)
        target_group_id = int(context.user_data.get("schedule_target_group_id", 0) or 0)
        if not quiz_id or not target_group_id:
            context.user_data["waiting_schedule_time"] = False
            await update.message.reply_text("❌ Schedule context is missing. Please load the quiz again.")
            return
        ...
        try:
            schedule_id = await create_group_schedule_async(
                quiz_id=quiz_id,
                owner_id=owner_id,
            ...
            row = await get_group_schedule_async(schedule_id)
            msg = await send_schedule_card(context.bot, row, live=False)
            if msg:
                await set_schedule_card_message_async(schedule_id, msg.message_id)
                card_note = ""
            else:
                card_note = "\n⚠️ Schedule saved; Telegram card will be recovered by the scheduler."
        except Exception:
            logger.exception("Group schedule creation failed.")
            await update.message.reply_text("❌ Failed to save the Group Schedule. Please try again.")
            return

        context.user_data["waiting_schedule_time"] = False
        context.user_data.pop("schedule_target_group_id", None)
        await update.message.reply_text(
            "✅ <b>Group Schedule saved</b>\n\n"
            f"📅 {format_schedule_time(scheduled_at)}\n"
            f"🆔 Schedule: <code>{schedule_id}</code>\n"
            "🔒 Start/Share stay locked until the scheduled time."
            f"{card_note}",
            parse_mode="HTML"
        )
        return
...
async def begin_quiz(
    ...
    try:
        ...
        source = quiz_data or context.user_data
...
async def post_init(
    ...
    # Persistent DB-backed GROUP scheduler. Pending schedules survive restart.
    schedule_task = asyncio.create_task(group_schedule_worker(application))
