...
def schedule_keyboard(schedule_id: int, short_code: str = "") -> InlineKeyboardMarkup:
    ...
    if short_code:
        rows.extend([
            [InlineKeyboardButton("▶️ Start this quiz", callback_data=f"sched_action_start_{schedule_id}_{short_code}")],
            [InlineKeyboardButton("👥 Start quiz in group", callback_data=f"sched_action_group_{schedule_id}_{short_code}")],
            [InlineKeyboardButton("🔗 Share quiz", callback_data=f"sched_action_share_{schedule_id}_{short_code}")],
        ])
    rows.extend([
    ...
    try:
        if target_type == "group":
            ...
            await edit_schedule_control_message(
                ...
                f"🖊 <b>{len(questions)} questions</b>\n\n"
                "🟢 <b>Quiz is now active. You can use the buttons below.</b>",
                schedule_keyboard(schedule_id, short_code)
            )
        else:
            await send_scheduled_channel_quiz(context, quiz, target_chat_id)
            await delete_reminder(context, schedule_id)
            await update_schedule_status(schedule_id, "COMPLETED")
            await delete_schedule_control_message(context, schedule_id)
    except Exception:
...
async def delete_schedule_control_message(context, schedule_id: int):
    if not db_pool:
        return
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
...
async def inline_query_handler(
    ...
    if loaded:
        ...
        pending_rows = await get_schedule_for_quiz(quiz_id)
        if pending_rows:
            sid = int(pending_rows[0]["id"])
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("▶️ Start this quiz", callback_data=f"sched_action_start_{sid}_{short_code}")],
                [InlineKeyboardButton("👥 Start quiz in group", callback_data=f"sched_action_group_{sid}_{short_code}")],
                [InlineKeyboardButton("🔗 Share quiz", callback_data=f"sched_action_share_{sid}_{short_code}")],
            ])
        else:
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("▶️ Start this quiz", url=start_link)],
...
async def button_handler(
    ...
    # Scheduled quiz buttons: locked until THIS schedule becomes active.
    if data.startswith("sched_action_"):
        parts = data.split("_", 3)
        if len(parts) != 4:
            await query.answer("❌ Invalid scheduled quiz button.", show_alert=True)
            return
        action = parts[2]
        rest = parts[3]
        try:
            schedule_id_str, code = rest.split("_", 1)
            schedule_id = int(schedule_id_str)
        except (ValueError, TypeError):
            await query.answer("❌ Invalid scheduled quiz.", show_alert=True)
            return
        ...
        if not row or row["quiz_id"] is None:
            await query.answer("❌ Schedule not found.", show_alert=True)
            return
        ...
        if row["status"] not in ("RUNNING",):
            await query.answer("❌ This scheduled quiz is no longer active.", show_alert=True)
            return
        ...
        if action == "group":
            if query.message.chat.type in ("group", "supergroup"):
                if not await is_target_chat_admin_or_owner(query.message.chat, query.from_user.id):
                    await query.answer("❌ Only the Group Admin or Owner can start the quiz here.", show_alert=True)
                    return
                owner_id, title, questions, short_code, quiz_id = loaded
                context.user_data.update({
                    "owner_id": owner_id, "quiz_id": quiz_id, "short_code": short_code,
                    "quiz_title": title, "questions": deepcopy(questions),
                    "qshuffle": False, "oshuffle": False, "explanation": True, "timer": 20
                })
                await query.answer("🚀 Starting quiz in this group...")
        ...
        if action == "share":
            # Open the quiz in the bot so the normal Share Quiz button can be used.
            await query.answer(url=f"https://t.me/{BOT_USERNAME}?start=quiz_{code}")
            return
