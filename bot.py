    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("""
                ...
                ON quiz_schedules(target_chat_id, status, scheduled_at);
...
# 6. SCHEDULE ENGINE
# ==========================================
def schedule_keyboard(schedule_id: int, short_code: str = "") -> InlineKeyboardMarkup:
    rows = []
    if short_code:
        rows.extend([
            [InlineKeyboardButton("▶️ Start this quiz", callback_data=f"sched_lock_start_{short_code}")],
            [InlineKeyboardButton("👥 Start quiz in group", callback_data=f"sched_lock_group_{short_code}")],
            [InlineKeyboardButton("🔗 Share quiz", callback_data=f"sched_lock_share_{short_code}")],
        ])
    rows.extend([
        [InlineKeyboardButton("✏️ Change Date & Time", callback_data=f"sched_edit_{schedule_id}")],
        [InlineKeyboardButton("🗑️ Cancel Schedule", callback_data=f"sched_cancel_{schedule_id}")],
    ])
...
async def get_schedule_for_quiz(quiz_id: int):
    if not db_pool:
        return []
    async with db_pool.acquire() as conn:
        return await conn.fetch(
            ...
            WHERE quiz_id=$1 AND status IN ('PENDING','WAITING')
...
async def has_pending_schedule_for_quiz(quiz_id: int) -> bool:
    rows = await get_schedule_for_quiz(quiz_id)
    return bool(rows)
...
async def create_schedule(
    ...
    async with db_pool.acquire() as conn:
        # Exact same destination/time is not allowed.
...
async def delete_reminder(context, schedule_id: int):
    ...
    try:
        await context.bot.delete_message(
            ...
            message_id=row["reminder_message_id"]
        )
...
async def send_scheduled_channel_quiz(context, quiz, target_chat_id: int):
...
async def run_one_schedule(context, row):
    ...
    # Group queue: never interrupt an already-running live quiz.
    if target_type == "group" and await redis_client.get(f"quiz_active:{target_chat_id}"):
        if not row["warning_sent"]:
            try:
                await context.bot.send_message(
                    target_chat_id,
                    "⚠️ <b>Quiz is currently running.</b> This quiz will start automatically when it ends.",
                    parse_mode="HTML"
                )
            except TelegramError:
                pass
    ...
    try:
        if target_type == "group":
            ...
            class ScheduleContext:
                pass
            # Reuse the existing begin_quiz engine with an isolated data map.
            ...
            if await redis_client.get(f"quiz_active:{target_chat_id}") is None:
                await update_schedule_status(schedule_id, "FAILED")
                return
            # Keep the schedule RUNNING until the existing Live Quiz Engine
            # finishes. finish_quiz() marks it COMPLETED and unlocks the flow.
            async with db_pool.acquire() as conn:
                await conn.execute(
            ...
            await send_scheduled_channel_quiz(context, quiz, target_chat_id)
            await delete_reminder(context, schedule_id)
            ...
            await edit_schedule_control_message(
                ...
                f"🖊 <b>{len(questions)} questions</b>\n\n"
                "✅ <b>This quiz has been shared in the scheduled Channel.</b>",
                None
            )
    except Exception:
...
async def process_due_schedules(context):
    ...
    for row in rows:
        # Atomically claim the schedule so a restart/multiple worker cannot execute twice.
        async with db_pool.acquire() as conn:
        ...
        if not claimed:
            continue
        # A live group may force it back to WAITING; run_one_schedule handles that.
        ...
        try:
            if db_pool:
                ...
                for row in rows:
                    try:
                        text = (
                            "⏳ <b>Quiz will start in 5 minutes. Get ready!</b>"
                            if row["target_type"] == "group"
                            else "⏳ <b>Quiz will be shared in 5 minutes. Get ready!</b>"
                        )
...
async def schedule_start_flow(query, context, target_type: str):
    ...
    if target_type == "group":
        context.user_data["schedule_target_type"] = "group"
        prompt = (
...
async def handle_schedule_target(update, context, raw_target: str):
    # Schedule targets intentionally accept ONLY a public username/link.
    # Chat IDs are not accepted for scheduling.
    ...
    if error_message:
        # Schedule intentionally accepts only username/public link as user input.
        if re.fullmatch(r"-?\d+", raw_target.strip()):
...
async def handle_schedule_time(update, context, text: str):
    ...
    if edit_id:
        async with db_pool.acquire() as conn:
            duplicate = await conn.fetchrow(
                """SELECT id FROM quiz_schedules
                   WHERE target_chat_id=$1 AND scheduled_at=$2
                     AND status IN ('PENDING','WAITING','RUNNING') AND id<>$3
                   LIMIT 1""",
                int(context.user_data["schedule_target_chat_id"]),
            ...
            await conn.execute(
                """UPDATE quiz_schedules
                   SET scheduled_at=$2, status='PENDING', warning_sent=FALSE,
                       reminder_message_id=NULL, settings=$3::jsonb, updated_at=CURRENT_TIMESTAMP
                   WHERE id=$1""",
                int(edit_id), scheduled_at.astimezone(timezone.utc),
                json.dumps(settings, ensure_ascii=False)
            )
        schedule_id = int(edit_id)
        error = None
    else:
        schedule_id, error = await create_schedule(
            ...
            control_message_id=context.user_data.get("schedule_control_message_id") or 0,
            settings=settings
        )
    if error == "duplicate":
        await update.message.reply_text(
    ...
    context.user_data.pop("schedule_edit_id", None)
    ...
    text_msg = (
        ...
        f"🔒 <b>This quiz will {'start automatically' if target_label == 'Group' else 'be shared automatically'} at the scheduled time.</b>\n"
        f"📅 <b>Scheduled for: {schedule_display(scheduled_at)}</b>\n"
        f"{('👥' if target_label == 'Group' else '📢')} <b>{safe_html(context.user_data['schedule_target_username'])}</b>"
    )
    try:
        if edit_id:
            await edit_schedule_control_message(
                ...
                schedule_id,
                text_msg,
                schedule_keyboard(schedule_id, context.user_data.get("short_code", ""))
            )
        else:
            control_message = await update.message.reply_text(
                text_msg,
                parse_mode="HTML",
                reply_markup=schedule_keyboard(schedule_id, context.user_data.get("short_code", ""))
            )
            async with db_pool.acquire() as conn:
                await conn.execute(
                    "UPDATE quiz_schedules SET control_chat_id=$2, control_message_id=$3, updated_at=CURRENT_TIMESTAMP WHERE id=$1",
                    schedule_id, update.effective_chat.id, control_message.message_id
                )
    except TelegramError:
        pass

    await update.message.reply_text(
        "✅ Schedule updated successfully." if edit_id else "✅ Schedule created successfully."
    )


async def schedule_user_authorized(query, context, row) -> bool:
    """Only an Owner/Admin of the scheduled target may manage it."""
    try:
...
async def edit_schedule_flow(query, context, schedule_id: int):
    if query.message.chat.type != "private":
        await query.answer("Please use private chat.", show_alert=True)
        return
    async with db_pool.acquire() as conn:
    ...
    if row["status"] not in ("PENDING", "WAITING"):
        await query.answer("❌ This schedule can no longer be changed.", show_alert=True)
        return
    if not await schedule_user_authorized(query, context, row):
        await query.answer("❌ Only the target Group/Channel Admin or Owner can change this schedule.", show_alert=True)
        return
    quiz = await load_quiz_for_schedule(row["quiz_id"])
    context.user_data["quiz_id"] = row["quiz_id"]
    context.user_data["owner_id"] = row["owner_id"]
    if quiz:
        context.user_data["quiz_title"] = quiz[1]
        context.user_data["questions"] = quiz[2]
        context.user_data["short_code"] = quiz[3]
        raw_settings = row.get("settings") or {}
        if isinstance(raw_settings, str):
            raw_settings = json.loads(raw_settings)
        context.user_data["qshuffle"] = bool(raw_settings.get("qshuffle", False))
        context.user_data["oshuffle"] = bool(raw_settings.get("oshuffle", False))
        context.user_data["explanation"] = bool(raw_settings.get("explanation", True))
        context.user_data["timer"] = int(raw_settings.get("timer", 20))
    context.user_data["schedule_edit_id"] = schedule_id
    context.user_data["schedule_target_type"] = row["target_type"]
    context.user_data["schedule_target_chat_id"] = row["target_chat_id"]
    context.user_data["schedule_target_username"] = row["target_username"]
    context.user_data["schedule_control_message_id"] = row["control_message_id"]
    context.user_data["waiting_schedule_date"] = True
    context.user_data["waiting_schedule_time"] = False
    await query.answer()
    await query.message.reply_text(
        "📅 <b>Send the new date</b> in this format:\n<code>YYYY-MM-DD</code>",
        parse_mode="HTML"
    )
...
async def cancel_schedule(query, context, schedule_id: int):
    ...
    if not row:
        ...
        return
    quiz = await load_quiz_for_schedule(row["quiz_id"])
    if not quiz or not await schedule_user_authorized(query, context, row):
    ...
    await query.message.reply_text("✅ <b>Schedule cancelled.</b>", parse_mode="HTML")
    await edit_schedule_control_message(
        context, schedule_id,
        f"🎲 <b>Quiz • {safe_html(quiz[1])}</b>\n\n"
        f"🖊 <b>{len(quiz[2])} questions</b>\n\n"
        "❌ <b>This schedule has been cancelled.</b>",
        None
    )
    try:
        raw_settings = row.get("settings") or {}
        if isinstance(raw_settings, str):
            try:
                ...
                raw_settings = {}
        await query.message.edit_reply_markup(reply_markup=build_settings_keyboard({
            "short_code": quiz[3],
            "qshuffle": bool(raw_settings.get("qshuffle", False)),
            "oshuffle": bool(raw_settings.get("oshuffle", False)),
            "explanation": bool(raw_settings.get("explanation", True)),
...
async def inline_query_handler(
    ...
    if loaded:
        _, title, questions, short_code, quiz_id = loaded
        ...
        pending_scheduled = await has_pending_schedule_for_quiz(quiz_id)
...
async def button_handler(
    ...
    # Scheduled-state locks for shared quiz buttons.
    if data.startswith("sched_lock_"):
        code = data.split("_", 3)[-1]
        loaded = await load_quiz_by_code_or_id(code)
        locked = bool(loaded and await has_pending_schedule_for_quiz(loaded[4]))
        if locked:
            await query.answer("❌ You can't do this unless it's scheduled.", show_alert=True)
            return
        if data.startswith("sched_lock_start_"):
            await query.answer(url=f"https://t.me/{BOT_USERNAME}?start=quiz_{code}")
            return
        if data.startswith("sched_lock_group_"):
            await query.answer(url=f"https://t.me/{BOT_USERNAME}?startgroup=quiz_{code}")
            return
        await query.answer(
            url=f"https://t.me/{BOT_USERNAME}?start=quiz_{code}"
        )
        return
    ...
    if data.startswith("sched_edit_"):
        try:
            schedule_id = int(data.split("_", 2)[2])
        except ValueError:
            await query.answer("❌ Invalid schedule.", show_alert=True)
            return
        await edit_schedule_flow(query, context, schedule_id)
        return
...
async def receive_text(
    ...
    # Schedule target/date/time inputs are private-chat only.
...
async def finish_quiz(
    ...
    try:
        ...
        # A scheduled Group quiz is considered complete only after the
        # existing Live Quiz Engine has finished and results were posted.
        async with db_pool.acquire() as conn:
            finished_schedule = await conn.fetchrow(
                """SELECT s.id FROM quiz_schedules s
                   WHERE s.target_chat_id=$1 AND s.target_type='group'
                     AND s.status='RUNNING'
                     AND s.quiz_id=$2
                   ORDER BY s.updated_at ASC, s.id ASC LIMIT 1""",
                target_chat_id, int(quiz_info.get("quiz_id") or 0)
            )
            if finished_schedule:
                await conn.execute(
                    "UPDATE quiz_schedules SET status='COMPLETED', updated_at=CURRENT_TIMESTAMP WHERE id=$1",
                    finished_schedule["id"]
                )
                try:
                    await edit_schedule_control_message(
                        context, int(finished_schedule["id"]),
                        f"🎲 <b>Quiz • {safe_html(title)}</b>\n\n"
                        f"🖊 <b>{total_q} questions</b>\n\n"
                        "✅ <b>This scheduled Group quiz has finished.</b>",
                        None
                    )
                except Exception:
                    logger.exception("Failed to update completed schedule card.")

        # If a scheduled Group quiz was waiting in the queue, start it now.
        try:
...
async def recover_waiting_schedules(context):
    if not db_pool:
        return
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            """SELECT * FROM quiz_schedules
               WHERE status='WAITING' AND target_type='group'
               ORDER BY scheduled_at ASC, id ASC LIMIT 100"""
        )
    for row in rows:
        if not await redis_client.get(f"quiz_active:{row['target_chat_id']}"):
            await run_one_schedule(context, row)
...
async def post_init(
    ...
    await recover_waiting_schedules(recovery_context)

    schedule_task = asyncio.create_task(
        schedule_reminder_worker(application)
    )
