    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("""
                ...
                CREATE INDEX IF NOT EXISTS idx_scheduled_quizzes_time
                ON scheduled_quizzes(scheduled_time);
            """)
...
# 18.5. SCHEDULER
# ==========================================
async def run_scheduled_quiz_job(
    chat_id: int,
    quiz_code: str,
    application: Application,
    schedule_id: int | None = None,
):
    """Start a saved quiz from an APScheduler job."""
    try:
        loaded = await load_quiz_by_code_or_id(str(quiz_code))
        if not loaded:
            logger.error(
                "Scheduled quiz %s not found for chat %s",
                quiz_code,
                chat_id,
            )
            if schedule_id and db_pool:
                async with db_pool.acquire() as conn:
                    await conn.execute(
                        "DELETE FROM scheduled_quizzes WHERE id=$1",
                        schedule_id,
                    )
            return
        ...
        # begin_quiz only needs bot + user_data, so scheduled jobs do not
        # depend on a user's ConversationHandler context.
        ...
        await begin_quiz(chat_id, context)
        ...
        logger.info(
            "Scheduled quiz started: id=%s chat=%s code=%s",
            schedule_id,
            chat_id,
            quiz_code,
        )
        ...
        logger.exception(
            "Scheduled quiz failed: chat=%s code=%s",
            chat_id,
            quiz_code,
        )
...
async def restore_scheduled_jobs(application: Application):
    """Restore future schedules after a Render restart/deploy."""
    if not db_pool:
        return
    ...
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            """
            SELECT id, chat_id, quiz_code, scheduled_time
    ...
    for row in rows:
        scheduler.add_job(
            ...
            trigger=DateTrigger(run_date=row["scheduled_time"]),
            args=[
                row["chat_id"],
                row["quiz_code"],
                application,
    ...
    logger.info("✅ Restored %d scheduled quiz job(s).", len(rows))
...
async def schedule_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 3:
        await update.message.reply_text(
            "❌ <b>Usage:</b> <code>/schedule QUIZ_CODE YYYY-MM-DD HH:MM</code>",
            parse_mode="HTML",
        )
        return

    quiz_code, date_str, time_str = (
        context.args[0],
        context.args[1],
        context.args[2],
    )
    ...
    if run_date <= datetime.now(ist_timezone):
        await update.message.reply_text(
    ...
    if not loaded:
        await update.message.reply_text(
            f"❌ Quiz code <code>{safe_html(quiz_code)}</code> not found.",
            parse_mode="HTML",
        )
        return
    ...
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            INSERT INTO scheduled_quizzes
                (chat_id, quiz_code, scheduled_time)
            VALUES ($1, $2, $3)
            RETURNING id
            """,
            chat.id,
    ...
    try:
        scheduler.add_job(
            ...
            args=[chat.id, quiz_code, context.application, schedule_id],
            id=f"sch_{schedule_id}",
            replace_existing=True,
        )
    except Exception:
        async with db_pool.acquire() as conn:
        ...
        logger.exception("Failed to add schedule %s", schedule_id)
        await update.message.reply_text(
    ...
    await update.message.reply_text(
        ...
        f"📌 <b>Quiz Code:</b> <code>{safe_html(quiz_code)}</code>\n"
...
async def unschedule_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "❌ <b>Usage:</b> <code>/unschedule SCHEDULE_ID</code>",
            parse_mode="HTML",
        )
        return
    ...
    async with db_pool.acquire() as conn:
        ...
        if not row:
            await update.message.reply_text(
                "❌ No active schedule found with this ID."
            )
            return
    ...
    try:
        scheduler.remove_job(f"sch_{schedule_id}")
    except Exception:
        pass
...
async def post_init(
    ...
    # Scheduler starts only after the asyncio loop and DB are ready.
