...
async def create_schedule_record(owner_id: int, quiz_id: int, target_chat_id: int,
                                 target_username: str, quiz_title: str,
    ...
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            (owner_id, quiz_id, target_chat_id, target_username, quiz_title,
...
async def process_schedule_text(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str) -> bool:
    ...
    if flow.get("stage") == "datetime":
        ...
        schedule_id = await create_schedule_record(
            ...
            chat.id,
            flow["target_username"], title, q, qshuffle, oshuffle, explanation, timer, dt
        )
        ...
        await update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("📅 My Schedules", callback_data="schedule_list")]
            ])
        )
        context.user_data.pop("schedule_flow", None)
        return True
        ...
        try:
            ...
            for row in reminder_rows:
                try:
                    ...
                    await db_pool.execute("UPDATE quiz_schedules SET reminder_sent=TRUE WHERE id=$1", row["id"])
                except TelegramError:
