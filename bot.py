...
IMPORT_PREVIEW_LIMIT = 20  # display-only preview constant; never limits parsing
...
schedule_wakeup_event = None
IST = ZoneInfo("Asia/Kolkata")

# Atomic live-answer update. Keeps the existing scoring/leaderboard rules
# while reducing the number of Redis round trips during answer bursts.
LIVE_ANSWER_LUA = r"""
local first = redis.call("SET", KEYS[1], "1", "NX", "EX", 7200)
if not first then
    return {0, 0, 0}
end
...
local score = tonumber(redis.call("HGET", KEYS[3], ARGV[1]) or "0")
if ARGV[3] == "1" then
    score = redis.call("HINCRBY", KEYS[3], ARGV[1], 1)
end

local total_time = redis.call("HINCRBYFLOAT", KEYS[4], ARGV[1], ARGV[4])
local composite_rank = (score * 1000000) - math.min(tonumber(total_time), 999999.0)
redis.call("ZADD", KEYS[5], composite_rank, ARGV[1])

return {1, score, total_time}
"""
...
async def can_manage_or_share_quiz_callback(query, context) -> bool:
    """Allow owner or a user who loaded a valid shared quiz in private chat.
    Group actions remain restricted to group owner/admin.
    Used only by Schedule and Share Polls flows.
    """
    chat = query.message.chat
    user_id = query.from_user.id
    ...
    if not quiz_id:
        await query.answer("❌ Please load or open a quiz first.", show_alert=True)
        return False

    if chat.type == "private":
        # The quiz was loaded through a valid saved-quiz/share path.
        # Owner is naturally included; shared users are allowed for these
        # two collaboration actions only.
        return True

    if chat.type in ["group", "supergroup"]:
        if not await is_target_chat_admin_or_owner(chat, user_id):
            await query.answer(
                "❌ Only the Group Admin or Owner can use this action.",
                show_alert=True
            )
            return False
        return True

    await query.answer("❌ This action is not allowed here.", show_alert=True)
    return False
...
async def init_infrastructure():
    ...
    try:
        # Keep the pool intentionally small for low-resource hosted PostgreSQL
        # plans. A large idle pool wastes DB resources and can trigger the
        # provider's compute/resource quota even when the bot is lightly used.
...
def parse_questions_detailed(text: str):
    ...
    for raw in lines:
        ...
        if numeric and cur and numeric_question_mode:
            # If the current numeric question already has a complete option set, a
            # following numeric line is a new question even when the previous answer
            # marker was missing. This prevents one malformed question from swallowing
            # the rest of a large batch.
            option_count_so_far = sum(1 for x in cur[1:] if re.match(r"^\s*(?:[A-Ja-j]|\d{1,2})\s*[\.\):\-]\s+", x))
            if saw_answer or option_count_so_far >= 4:
...
def validate_questions_for_telegram(questions: list, include_number_prefix: bool = False):
    ...
    for i,q in enumerate(questions,1):
        ...
        # The live renderer safely trims the display-only numbering prefix when needed.
        # Do not reject an otherwise valid 300-character source question merely because
        # the prefix would push the rendered copy over Telegram's 300-char limit.
        if len(question)>MAX_POLL_QUESTION_LEN:
            errors.append(f"Q{i}: question is too long ({len(question)}/{MAX_POLL_QUESTION_LEN} characters)")
...
async def create_schedule_record(owner_id: int, quiz_id: int, target_chat_id: int,
    ...
    async with db_pool.acquire() as conn:
        ...
        # Wake the scheduler immediately when a new schedule is created.
        # This avoids constant DB polling while still reacting promptly to
        # schedules created after the worker has gone idle.
        if schedule_wakeup_event is not None:
            schedule_wakeup_event.set()
        return schedule_id
...
async def schedule_worker(application: Application):
    """Low-compute scheduler that preserves one-time scheduling semantics.

    The old implementation queried PostgreSQL every few seconds forever. That
    kept the Neon compute active even when there were no scheduled quizzes.
    This worker instead sleeps until the next schedule is relevant and is
    explicitly woken when a new schedule is created.
    """
    global schedule_wakeup_event
    if schedule_wakeup_event is None:
        schedule_wakeup_event = asyncio.Event()

    logger.info("📅 Schedule worker STARTED (low-compute mode).")

    while True:
        try:
            if not db_pool:
                logger.error("⏰ Scheduler has no DB pool; retrying in 30s.")
                await asyncio.sleep(30)
                continue

            now = datetime.now(timezone.utc)
            # Clear before querying: if a schedule is created during/after the
            # query, its event signal remains set and wakes the worker.
            schedule_wakeup_event.clear()
            ...
            if next_row is None:
                # No scheduled work: sleep without touching PostgreSQL. A new
                # schedule creation wakes this event immediately.
                try:
                    await asyncio.wait_for(schedule_wakeup_event.wait(), timeout=900)
                except asyncio.TimeoutError:
                    pass
                continue

            next_at = next_row["scheduled_at"]
            if next_at.tzinfo is None:
                next_at = next_at.replace(tzinfo=timezone.utc)
            next_at = next_at.astimezone(timezone.utc)

            # Wake at most five minutes before the next schedule so the
            # reminder window is preserved. For schedules already due, process
            # immediately.
            wake_at = next_at - timedelta(minutes=5)
            delay = max(0.0, (wake_at - now).total_seconds())

            if delay > 0:
                try:
                    await asyncio.wait_for(schedule_wakeup_event.wait(), timeout=delay)
                except asyncio.TimeoutError:
                    pass
                continue

            # We are inside the reminder/due window. Poll only while there is
            # active scheduled work, then return to the long sleep above.
            now = datetime.now(timezone.utc)
            ...
            # Keep the existing one-time scheduler behavior, but only poll
            # during the active five-minute window.
            try:
                await asyncio.wait_for(schedule_wakeup_event.wait(), timeout=15)
            except asyncio.TimeoutError:
                pass
...
def build_start_keyboard():
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton(
                "👥 Join Quiz Community",
                url="https://t.me/alok_advance_quizbot_group"
            )
        ],
        [
            InlineKeyboardButton(
...
def build_creation_done_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔎 Review Invalid", callback_data="import_review_invalid")],
        [InlineKeyboardButton("❌ Cancel", callback_data="cancel_creation")]
    ])


def build_import_preview_keyboard():
    # Kept as a compatibility helper for older callback paths. There is no Add All action.
    return build_creation_done_keyboard()
...
async def help_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_text = (
        "🤖 <b>Welcome to Alok Advance Quiz Bot Help Center</b>\n"
        "Here is the complete list of available commands and controls:\n\n"
        "📌 <b>Quiz Creation &amp; Setup</b>\n"
        "• /create – Start creating a new quiz\n"
        "• /done – Finish adding questions and save quiz\n"
        "• /quizzes – View &amp; manage all your saved quizzes\n"
        "• /resetquiz – Reset current active quiz state\n"
        "• /cancel – Cancel active process\n\n"
        "📅 <b>Group &amp; Channel Management</b>\n"
        "• /schedule – Schedule currently loaded quiz in a group\n"
        "• /schedules – View &amp; manage pending scheduled quizzes\n"
        "• /postchannel – Send saved questions directly as native Telegram Quiz Polls\n\n"
        "⚙️ <b>Quiz Execution Control</b>\n"
        "• /stop or /pause – Pause or stop an ongoing live quiz\n"
...
async def done_command(
    ...
    await update.message.reply_text(
        ...
        f"📋 Questions: {len(questions)}\n"
        f"👤 Created by: <b>{safe_html(getattr(context.user_data.get('creator_user'), 'full_name', '') or 'Unknown')}</b>"
        + (f" (@{safe_html(getattr(context.user_data.get('creator_user'), 'username', ''))})" if getattr(context.user_data.get('creator_user'), 'username', '') else "") + "\n\n"
        f"Share Code:\n"
        f"<code>{safe_html(share_link)}</code>",
        parse_mode="HTML",
        reply_markup=build_settings_keyboard(
...
async def publish_all_polls(
    ...
    # publish_all_polls normally runs as a background task. Keep a fallback
    # for any direct/internal caller.
    if not context.user_data.get("publishing_polls"):
        context.user_data["publishing_polls"] = True
    cancel_event = context.user_data.setdefault(
        "publish_cancel_event", asyncio.Event()
    )
    ...
    try:
        ...
        try:
            progress_message = await query.message.reply_text(
                "📤 <b>Sending Polls...</b>\n\n"
                f"0/{total}\n\n"
                "You can stop the publishing job at any time.",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("❌ Cancel Publishing", callback_data="cancel_publish")]
                ])
            )
        except TelegramError:
            progress_message = None
            ...
            try:
                ...
                try:
                    await asyncio.wait_for(
                        cancel_event.wait(),
                        timeout=float(e.retry_after) + 1.0
                    )
                    break
                except asyncio.TimeoutError:
                    pass
        ...
        if progress_message:
            try:
                if was_cancelled:
                    await progress_message.edit_text(
                        "🛑 <b>Poll Publishing Cancelled</b>\n\n"
                        f"📋 Total: {total}\n"
                        f"✅ Sent before cancel: {sent_count}\n"
        ...
        context.user_data.pop("publish_cancel_event", None)
...
async def button_handler(
    ...
    if data == "schedule_quiz":
        if not await can_manage_or_share_quiz_callback(query, context):
            return
    ...
    if data == "confirm_publish_polls":
        ...
        # Run the long publishing job in the background so the bot continues
        # processing other users while a large poll batch is being sent.
        if context.user_data.get("publishing_polls"):
            await query.answer("⏳ Polls are already being sent.", show_alert=True)
            return
        context.user_data["publishing_polls"] = True
        context.user_data["publish_cancel_event"] = asyncio.Event()
        task = asyncio.create_task(publish_all_polls(query, context))
        track_task(task)
        await query.answer("🚀 Poll publishing started in background.")
        return
    ...
    # Cancel publishing / cancel before confirmation
    if data == "cancel_publish":
        if not await can_manage_or_share_quiz_callback(query, context):
            return

        cancel_event = context.user_data.get("publish_cancel_event")
        if context.user_data.get("publishing_polls") and cancel_event:
            cancel_event.set()
            context.user_data["waiting_publish_target"] = False
            await query.answer("🛑 Stopping poll publishing...")
            try:
    ...
    if data == "import_preview_back":
        await query.answer()
        await query.message.reply_text(
            "📝 <b>Questions Processed</b>\n\n"
            f"📊 Total Questions: {len(context.user_data.get('temp_questions', []))} / {MAX_QUIZ_QUESTIONS}\n"
            f"⚠️ Invalid/Skipped: {len(context.user_data.get('pending_import_invalid', []))}\n\n"
            "You can send more questions or upload another file.\n"
            "When you're finished, tap Done Adding Questions.",
            parse_mode="HTML", reply_markup=build_creation_done_keyboard()
        )
        return

    if data == "import_add_all":
        await query.answer("Questions are added automatically. No Add All action is needed.", show_alert=True)
        return
...
async def _queue_import_preview(update, context, questions, invalid_items, source_name="Import"):
    """Process a complete parsed batch immediately. No fixed batch/preview limit is applied.

    Valid questions are appended to the active draft immediately. Invalid items are
    accumulated so Review Invalid can show every rejected question across messages/files.
    The 1000-question cap is enforced per question, allowing the valid portion of a
    mixed batch to be added while every overflow item is reported individually.
    """
    current = context.user_data.setdefault("temp_questions", [])
    previous_invalid = context.user_data.setdefault("pending_import_invalid", [])
    start_total = len(current)
    added = 0
    processed_invalid = []

    # Validate Telegram constraints against the source question itself. Runtime rendering
    # safely handles the numbering prefix, so a 300-char source question is not rejected
    # merely because [N/total] is prepended later.
    for item in (invalid_items or []):
        item = dict(item)
        item["index"] = start_total + int(item.get("index", 1))
        processed_invalid.append(item)

    for offset, q in enumerate(questions or [], 1):
        if len(current) >= MAX_QUIZ_QUESTIONS:
            processed_invalid.append({
                "index": start_total + offset,
                "question": str(q.get("question", ""))[:160],
                "reason": f"Total quiz limit exceeded ({MAX_QUIZ_QUESTIONS} questions maximum)",
                "block": ""
            })
            continue
        q_errors = validate_questions_for_telegram([q], include_number_prefix=False)
        if q_errors:
            processed_invalid.append({
                "index": start_total + offset,
                "question": str(q.get("question", ""))[:160],
                "reason": q_errors[0].split(": ", 1)[1] if ": " in q_errors[0] else q_errors[0],
                "block": ""
            })
            continue
        current.append(q)
    ...
    previous_invalid.extend(processed_invalid)
    context.user_data["pending_import_source"] = source_name
    context.user_data["invalid_page"] = 0

    total = len(current)
    invalid_total = len(previous_invalid)
    await update.message.reply_text(
        "📝 <b>Questions Processed</b>\n\n"
        f"✅ Added This Time: {added}\n"
        f"📊 Total Questions: {total} / {MAX_QUIZ_QUESTIONS}\n"
        f"⚠️ Invalid/Skipped: {invalid_total}\n\n"
        "You can send more questions or upload another file.\n"
        "When you're finished, tap Done Adding Questions.",
        parse_mode="HTML",
...
async def receive_text(
    ...
    if context.user_data.get("waiting_title"):
        ...
        await update.message.reply_text(
            "📝 <b>Send Your Questions</b>\n\n"
            "You can directly paste your questions in standard text format or upload a file (.txt, .pdf, .csv). The bot will automatically parse and add them to your quiz.\n\n"
            "<b>Supported Format Example:</b>\n\n"
            "<code>Question: What is the capital of India?\nA) Mumbai\nB) Kolkata\nC) New Delhi\nD) Chennai\nCorrect: C\nExplanation: New Delhi is the capital of India.</code>",
            parse_mode="HTML"
        )
        return
...
async def poll_answer_handler(
    ...
    # The mapping is required before the chat-specific state keys are known.
    ...
    state_pipe = redis_client.pipeline(transaction=False)
    state_pipe.hgetall(f"quiz_state:{chat_key}")
    state_pipe.get(f"quiz_cancelled:{chat_key}")
    state_pipe.get(f"quiz_active:{chat_key}")
    current_state, cancelled, active = await state_pipe.execute()
    ...
    if not active and not mapping_session_id:
        return
    ...
    # Duplicate protection, participant registration, score/time updates and
    # leaderboard update are now one atomic Redis operation.
...
async def post_init(
    ...
    if not WEBHOOK_URL:
        ...
        try:
            ...
            if webhook_info.url:
                logger.warning(
                    "⚠️ Existing Telegram webhook detected while WEBHOOK_URL is empty; clearing it before polling: %s",
                    webhook_info.url
                )
...
def main():
    ...
    # This deployment is intentionally webhook-only.
    # Never fall back to polling: Telegram does not allow getUpdates while
    # a webhook is active, and the live bot is intended to use Render webhook delivery.
    if not WEBHOOK_URL:
        raise RuntimeError(
            "WEBHOOK_URL is required for this deployment. "
