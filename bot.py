...
schedule_worker_task: Optional[asyncio.Task] = None
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("""
                ...
                CREATE INDEX IF NOT EXISTS idx_quiz_schedules_group_time
                ON quiz_schedules(group_chat_id, scheduled_at);
...
def format_schedule_datetime(dt: datetime) -> str:
    local_dt = dt.astimezone(SCHEDULE_TIMEZONE)
    return local_dt.strftime("%d %B %Y, %I:%M %p IST")


def build_schedule_card(title: str, scheduled_at: datetime, live: bool = False):
    if live:
        text = (
    ...
    text = (
        f"📚 <b>{safe_html(title)}</b>\n\n"
        f"📅 <b>{format_schedule_datetime(scheduled_at)}</b>\n\n"
        "This quiz will automatically start at the scheduled time."
    )
...
async def verify_and_prepare_schedule(
    update: Update,
    ...
    if not questions:
        ...
        await update.message.reply_text("❌ No quiz is loaded. Please load a quiz first.")
        return

    await update.message.reply_text("🔍 Verifying group and admin permissions...")
    ...
    if chat.type not in ["group", "supergroup"]:
        await update.message.reply_text("❌ Schedule Quiz works only for Groups and Supergroups.")
        return

    if not await is_target_chat_admin_or_owner(chat, update.effective_user.id):
        await update.message.reply_text(
            "❌ Verification failed. Only a Group Admin or Owner can schedule a quiz in this group."
        )
        return

    context.user_data["waiting_schedule_target"] = False
    context.user_data["schedule_group_id"] = chat.id
    context.user_data["schedule_group_title"] = chat.title or "Group"
    context.user_data["waiting_schedule_date"] = True

    await update.message.reply_text(
        "✅ Verification successful.\n\n"
        "Now send the date for the schedule.\n"
        "Format: <code>YYYY-MM-DD</code>\n\n"
        "Example: <code>2026-08-25</code>",
        parse_mode="HTML"
    )


async def prepare_schedule_date(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
    raw_date: str
):
    parsed = parse_schedule_date(raw_date)
    if not parsed:
        await update.message.reply_text(
            "❌ Invalid date. Please use <code>YYYY-MM-DD</code>.\n\nExample: <code>2026-08-25</code>",
            parse_mode="HTML"
        )
        return
    ...
    if parsed < today:
        await update.message.reply_text("❌ The schedule date cannot be in the past.")
        return

    context.user_data["waiting_schedule_date"] = False
    context.user_data["schedule_date"] = parsed.isoformat()
    context.user_data["waiting_schedule_time"] = True

    await update.message.reply_text(
        "Now send the time.\n"
        "India time (IST) is used automatically.\n\n"
        "Format: <code>HH:MM</code> or <code>08:00 PM</code>\n"
...
async def create_schedule(
    ...
    if not parsed_time:
        await update.message.reply_text(
            "❌ Invalid time. Please use <code>HH:MM</code> or <code>08:00 PM</code>.",
            parse_mode="HTML"
        )
        return

    group_id = context.user_data.get("schedule_group_id")
    ...
    if not group_id or not date_str or not quiz_id or not questions:
        context.user_data.pop("waiting_schedule_time", None)
        await update.message.reply_text("❌ Schedule information is incomplete. Please load the quiz again.")
        return
    ...
    if local_dt <= now:
        await update.message.reply_text("❌ The scheduled date and time must be in the future.")
        return
    ...
    settings = {
        ...
        "owner_id": int(context.user_data.get("owner_id", update.effective_user.id)),
    }

    async with db_pool.acquire() as conn:
        # Serialize schedule creation per group so two simultaneous requests
        # cannot both pass the 5-minute-gap check.
        await conn.execute(
            "SELECT pg_advisory_xact_lock($1::bigint)",
            int(group_id)
        )

        # Keep at least a 5-minute gap between scheduled start times in the same group.
        conflict = await conn.fetchrow(
            """
            SELECT id, scheduled_at
            FROM quiz_schedules
            WHERE group_chat_id=$1
              AND status IN ('scheduled', 'waiting', 'starting')
        ...
        if conflict:
            conflict_dt = conflict["scheduled_at"].astimezone(SCHEDULE_TIMEZONE)
            await update.message.reply_text(
                "❌ Schedule could not be created.\n\n"
                "Another quiz is already scheduled too close to this time in the same group. "
                "Keep at least a 5-minute gap between schedules.\n\n"
                f"Existing schedule: <b>{format_schedule_datetime(conflict_dt)}</b>",
                parse_mode="HTML"
            )
            return
        ...
        row = await conn.fetchrow(
            ...
            (quiz_id, group_chat_id, creator_id, scheduled_at, title, settings, status)
    ...
    card_text, card_markup = build_schedule_card(title, scheduled_at)
    ...
    try:
        ...
        await db_pool.execute(
            "UPDATE quiz_schedules SET status='failed' WHERE id=$1",
            schedule_id
        )
        logger.exception("Failed to post schedule card to group %s", group_id)
        await update.message.reply_text(
            "❌ Schedule was not completed because the bot could not post the schedule card in the group.\n\n"
            f"Telegram: {safe_html(str(exc))}",
            parse_mode="HTML"
        )
        return
    ...
    group_title = context.user_data.get("schedule_group_title", "Group")
    context.user_data["waiting_schedule_time"] = False
    for key in [
        "schedule_group_id", "schedule_group_title", "schedule_date",
        "schedule_quiz_id", "waiting_schedule_target", "waiting_schedule_date",
    ...
    await update.message.reply_text(
        ...
        f"👥 Group: <b>{safe_html(group_title)}</b>\n"
        f"📅 Time: <b>{format_schedule_datetime(scheduled_at)}</b>",
        parse_mode="HTML"
    )


async def cancel_schedule_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    chat = query.message.chat
    user = query.from_user

    if chat.type not in ["group", "supergroup"]:
        await query.answer("❌ This schedule can only be cancelled from its group.", show_alert=True)
        return

    if not await is_target_chat_admin_or_owner(chat, user.id):
        await query.answer("❌ Only a Group Admin or Owner can cancel this schedule.", show_alert=True)
        return
...
class ScheduledQuizContext:
    def __init__(self, application, user_data: dict):
        self.bot = application.bot
        self.user_data = user_data


async def start_scheduled_quiz(application: Application, row):
    schedule_id = int(row["id"])
    group_id = int(row["group_chat_id"])
    quiz_id = int(row["quiz_id"])
    title = row["title"]

    loaded = await load_quiz_by_code_or_id(str(quiz_id))
    if not loaded:
        await db_pool.execute(
            "UPDATE quiz_schedules SET status='failed' WHERE id=$1",
            schedule_id
        ...
        try:
            await application.bot.send_message(
                group_id,
                "❌ Scheduled quiz could not start because the saved quiz is no longer available."
            )
        except TelegramError:
            pass
        return

    owner_id, loaded_title, questions, short_code, loaded_quiz_id = loaded
    settings = row["settings"]
    if isinstance(settings, str):
        settings = json.loads(settings)

    # The scheduler only starts the existing quiz engine; it does not create a second quiz flow.
    schedule_user_id = int(settings.get("owner_id") or row["creator_id"])
    user_data = {
        "owner_id": owner_id,
        "quiz_id": loaded_quiz_id,
        "short_code": short_code,
        "quiz_title": loaded_title or title,
        "questions": questions,
        ...
        "schedule_creator_id": schedule_user_id,
    }

    # Wait for an already-running quiz in this group to finish.
    # The schedule remains cancellable while it is waiting.
    while await redis_client.get(f"quiz_active:{group_id}"):
        current_status = await db_pool.fetchval(
            "SELECT status FROM quiz_schedules WHERE id=$1",
            schedule_id
        )
    ...
    await db_pool.execute(
        "UPDATE quiz_schedules SET status='running' WHERE id=$1",
        schedule_id
    )
    ...
    try:
        await application.bot.edit_message_text(
            ...
            text=(
                f"📚 <b>{safe_html(loaded_title or title)}</b>\n\n"
                "🟢 <b>LIVE</b>\n\n"
                "The quiz has started."
            ),
            parse_mode="HTML"
    ...
    schedule_context = ScheduledQuizContext(application, user_data)
    try:
        await begin_quiz(group_id, schedule_context)
    except Exception:
        logger.exception("Scheduled quiz start failed. schedule_id=%s", schedule_id)
        await db_pool.execute(
            "UPDATE quiz_schedules SET status='failed' WHERE id=$1",
            schedule_id
        )
    ...
    # begin_quiz is intentionally reused. If another start lock briefly won,
    # verify that the quiz really became active before considering the schedule started.
    if not await redis_client.get(f"quiz_active:{group_id}"):
        await db_pool.execute(
...
async def schedule_worker(application: Application):
    logger.info("🗓 Group schedule worker started. Timezone: Asia/Kolkata")
    while True:
        try:
            now_utc = datetime.now(timezone.utc)

            # Send each 5-minute reminder only once.
            reminder_rows = await db_pool.fetch(
                """
                SELECT id, group_chat_id, title, scheduled_at
                FROM quiz_schedules
            ...
            for row in reminder_rows:
                # The row may already be running/cancelled when the loop reaches it.
                updated = await db_pool.execute(
                    """
                    UPDATE quiz_schedules
                    SET reminder_sent=TRUE
                    WHERE id=$1 AND reminder_sent=FALSE AND status IN ('scheduled','waiting')
                    """,
                    int(row["id"])
                )
                if updated != "UPDATE 1":
                ...
                try:
                    await application.bot.send_message(
                        int(row["group_chat_id"]),
                        "All students get ready, the quiz will start in 5 minutes."
                    )
                except TelegramError:
                    logger.exception("Failed to send schedule reminder. schedule_id=%s", row["id"])

            due_rows = await db_pool.fetch(
                """
                SELECT id, quiz_id, group_chat_id, creator_id, scheduled_at, title, settings, card_message_id
                FROM quiz_schedules
                WHERE status IN ('scheduled','waiting')
                  AND scheduled_at <= $1
            ...
            for row in due_rows:
                # Claim the schedule so two worker iterations/processes cannot start it twice.
                claimed = await db_pool.fetchrow(
                    """
                    UPDATE quiz_schedules
                    SET status='starting'
                    WHERE id=$1 AND status IN ('scheduled','waiting')
                    RETURNING id, quiz_id, group_chat_id, creator_id, scheduled_at, title, settings, card_message_id
                    """,
                    int(row["id"])
                )
                if claimed:
                    task = asyncio.create_task(start_scheduled_quiz(application, claimed))
                    track_task(task)

            await asyncio.sleep(1)
        except asyncio.CancelledError:
            logger.info("🛑 Group schedule worker stopped.")
            raise
        except Exception:
...
async def button_handler(
    ...
    if data == "schedule_quiz":
        if query.message.chat.type != "private":
            await query.answer(
                "📅 Please use Schedule Quiz from a private chat.",
                show_alert=True
            )
            return
        ...
        await query.answer()
        await query.message.reply_text(
            "Now send the username, link, or Chat ID of the group in which you want to schedule the quiz.\n\n"
            "Example:\n"
            "<code>@mygroup</code>\n"
    ...
    # Cancel a group schedule
    if data == "cancel_schedule":
        await cancel_schedule_callback(update, context)
        return
...
async def receive_text(
    ...
    # Schedule flow is handled before the existing publish/title/question states.
    if context.user_data.get("waiting_schedule_target"):
        await verify_and_prepare_schedule(update, context, text)
        return
    ...
    if context.user_data.get("waiting_schedule_time"):
        await create_schedule(update, context, text)
        return
...
async def finish_quiz(
    ...
    try:
        ...
        schedule_id = int(quiz_info.get("schedule_id") or 0)
        ...
        if schedule_id > 0 and db_pool:
            try:
                await db_pool.execute(
                    "UPDATE quiz_schedules SET status='completed' WHERE id=$1",
                    schedule_id
                )
            except Exception:
                logger.exception("Failed to mark schedule %s as completed", schedule_id)
...
async def post_init(
    ...
    # Recover schedules after a process restart.
    # A schedule that was being claimed but had not started yet can safely
    # return to waiting. A schedule marked running is also recoverable if
    # Redis no longer contains an active quiz for that group.
    now_utc = datetime.now(timezone.utc)
    await db_pool.execute(
        """
        UPDATE quiz_schedules
    ...
    for running_row in running_rows:
        group_id = int(running_row["group_chat_id"])
        if not await redis_client.get(f"quiz_active:{group_id}"):
            await db_pool.execute(
                "UPDATE quiz_schedules SET status='waiting' WHERE id=$1 AND status='running'",
                int(running_row["id"])
            )
