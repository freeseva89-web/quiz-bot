...
async def execute_due_schedule(application: Application, schedule):
    ...
    try:
        ...
        if schedule["target_type"] == "channel":
            # A channel schedule uses the existing schedule card. At the due
            # time it becomes LIVE and its Start/Share buttons are unlocked.
        ...
        # Important: persist LIVE before editing Telegram. If Telegram has a
        # transient edit error, the schedule must not remain RUNNING forever.
        await mark_schedule_status(schedule_id, "LIVE")
