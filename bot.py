...
async def resolve_target_chat(bot, raw_target: str):
    ...
    if target is None:
        return None, (
            "❌ Target पहचान नहीं पाया।\n\n"
            "इनमें से कोई एक भेजें:\n"
            "• @channelusername\n"
            "• Channel/Group का public link\n"
            "• Chat ID जैसे -1001234567890\n\n"
            "⚠️ Private invite link (+...) से Bot API target resolve नहीं कर सकता। "
            "ऐसे chat के लिए Chat ID दें।"
        )
    ...
    try:
        ...
        return None, (
            "❌ Target नहीं मिला या Bot को उस target तक access नहीं है।\n\n"
            "कृपया check करें:\n"
            "• Link/Username सही है\n"
            "• Bot उस Channel/Group में मौजूद है\n"
            "• Bot को आवश्यक admin/send permission मिली है\n"
            "• जरूरत हो तो Chat ID इस्तेमाल करें\n\n"
            f"Telegram: {safe_html(str(e))}"
        )

    if chat.type not in ["channel", "group", "supergroup"]:
        return None, "❌ केवल Channel, Group या Supergroup को target बनाया जा सकता है।"
    ...
    try:
        ...
        return None, (
            "❌ Bot की membership/permission verify नहीं हो सकी।\n\n"
            f"{safe_html(str(e))}"
        )

    if bot_member.status not in ["creator", "administrator"]:
        return None, (
            "❌ Bot इस target का Administrator नहीं है।\n\n"
            "पहले Bot को Channel/Group में admin बनाइए और फिर दोबारा try करें।"
        )
    ...
    if chat.type == "channel":
        ...
        if can_post is False:
            return None, (
                "❌ Bot Channel में post नहीं कर सकता।\n\n"
                "Bot को Channel Admin बनाकर "
...
async def start_publish_polls(
    ...
    await query.message.reply_text(
        "📤 <b>Share Polls</b>\n\n"
        "अब Channel/Group का इनमें से कोई एक भेजें:\n\n"
        "🔗 Public Link:\n"
        ...
        "⚠️ Private invite link (+...) की जगह Chat ID दें।\n\n"
...
async def verify_and_prepare_publish(
    ...
    if not context.user_data.get("questions"):
        ...
        await update.message.reply_text(
            "❌ कोई questions loaded नहीं हैं।"
        )
        return

    await update.message.reply_text(
        "🔍 Target verify किया जा रहा है..."
    )
    ...
    await update.message.reply_text(
        ...
        "हर question एक अलग <b>native Telegram Quiz Poll</b> "
        "के रूप में भेजा जाएगा।\n\n"
        "Quiz Title की जरूरत नहीं है।\n"
        "Correct option <b>✅</b> से automatically सेट होगा।",
        parse_mode="HTML",
        reply_markup=keyboard
    )
...
async def publish_all_polls(
    ...
    try:
        ...
        for index, q in enumerate(questions, start=1):
            try:
                ...
                if not (
                    ...
                    raise ValueError(
                        "Correct option index is invalid."
                    )
        ...
        if failed_items:
            ...
            await query.message.reply_text(
                "⚠️ <b>कुछ Poll भेजे नहीं जा सके।</b>\n\n"
                f"{preview}",
                parse_mode="HTML"
            )
        ...
        await query.message.reply_text(
            "❌ Poll publishing रोक दिया गया क्योंकि target "
            "से Telegram error आया:\n\n"
            f"{safe_html(str(e))}",
            parse_mode="HTML"
        )
...
async def button_handler(
    ...
    if data == "help":
        ...
        help_text = (
            ...
            "<b>Correct answer:</b> केवल <code>✅</code>\n\n"
            "📤 Share Polls से questions को "
            "native Telegram Quiz Polls में "
            "Channel/Group में भेज सकते हैं।"
        )
    ...
    if data == "publish_polls":
        if query.message.chat.type != "private":
            await query.answer(
                "📤 Share Polls केवल private chat से करें।",
                show_alert=True
            )
            return
...
async def receive_text(
    ...
    if context.user_data.get("waiting_title"):
        ...
        await update.message.reply_text(
            ...
            "Correct answer पर केवल <code>✅</code> लगाएँ।\n\n"
            "Example:\n\n"
            "<code>Q1. भारत की राजधानी क्या है?\n"
            "A. मुंबई\n"
            "B. कोलकाता\n"
            "C. ✅ नई दिल्ली\n"
            "D. चेन्नई</code>\n\n"
            "आप एक batch में कई questions भेज सकते हैं।",
            parse_mode="HTML"
        )
        return
    ...
    if context.user_data.get("waiting_questions"):
        ...
        if not questions:
            await update.message.reply_text(
                "❌ Valid question format नहीं मिला।\n\n"
                "हर question में exactly एक <code>✅</code> "
                "correct option पर लगाएँ।",
                parse_mode="HTML"
            )
            return
        ...
        await update.message.reply_text(
            ...
            "और questions भेजें या नीचे "
            "<b>Done Adding Questions</b> दबाएँ।",
            parse_mode="HTML",
    ...
    try:
        ...
        while index <= total_questions:
            ...
            try:
                ...
                logger.exception(
                    "Poll delivery error in loop."
                )

                await context.bot.send_message(
                    chat_id,
                    "❌ Poll delivery failed due to "
                    "rate limits or connection loss."
                )
        ...
        if not paused_exit and not reset_exit:
            await finish_quiz(
                chat_key,
                context,
                chat_id=chat_id
...
async def postchannel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type != "private":
        await update.message.reply_text("📤 /postchannel केवल private chat में इस्तेमाल करें।")
        return

    questions = context.user_data.get("questions", [])
    if not questions:
        await update.message.reply_text(
            "❌ पहले कोई Quiz load/create करें, फिर /postchannel चलाएँ।"
        )
        return
    ...
    await update.message.reply_text(
        "📤 <b>Share Polls</b>\n\n"
        "Channel/Group का इनमें से कोई एक भेजें:\n\n"
