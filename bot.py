    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("""
                ...
                ALTER TABLE quiz_results
                ADD COLUMN IF NOT EXISTS correct INT DEFAULT 0;
                ...
                ON scheduled_quizzes(status, run_at);
                CREATE INDEX IF NOT EXISTS idx_scheduled_quizzes_owner
                ON scheduled_quizzes(owner_id, status);
            """)
...
# ==========================================
# 6. SCHEDULING / RESULT MANAGEMENT
# ==========================================
async def create_schedule(owner_id: int, chat_id: int, quiz_id: int, run_at: datetime) -> int:
    if run_at.tzinfo is None:
        run_at = run_at.replace(tzinfo=timezone.utc)
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            """INSERT INTO scheduled_quizzes
               (quiz_id, owner_id, chat_id, run_at)
               VALUES ($1, $2, $3, $4) RETURNING id""",
            quiz_id, owner_id, chat_id, run_at
        )
        return int(row["id"])

async def list_schedules(owner_id: int, chat_id: Optional[int] = None):
    async with db_pool.acquire() as conn:
        if chat_id is None:
            return await conn.fetch(
                """SELECT s.id, s.quiz_id, q.title, s.chat_id, s.run_at, s.status
                   FROM scheduled_quizzes s JOIN quizzes q ON q.id=s.quiz_id
                   WHERE s.owner_id=$1 AND s.status='pending'
                   ORDER BY s.run_at LIMIT 50""", owner_id)
        return await conn.fetch(
            """SELECT s.id, s.quiz_id, q.title, s.chat_id, s.run_at, s.status
               FROM scheduled_quizzes s JOIN quizzes q ON q.id=s.quiz_id
               WHERE s.owner_id=$1 AND s.chat_id=$2 AND s.status='pending'
               ORDER BY s.run_at LIMIT 50""", owner_id, chat_id)

async def cancel_schedule(schedule_id: int, owner_id: int) -> bool:
    async with db_pool.acquire() as conn:
        result = await conn.execute(
            """UPDATE scheduled_quizzes SET status='cancelled'
               WHERE id=$1 AND owner_id=$2 AND status='pending'""",
            schedule_id, owner_id
        )
    return result == "UPDATE 1"

async def claim_due_schedules(limit: int = 20):
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            rows = await conn.fetch(
                """SELECT s.id, s.quiz_id, s.owner_id, s.chat_id, s.run_at,
                          q.title, q.questions, q.short_code
                   FROM scheduled_quizzes s
                   JOIN quizzes q ON q.id=s.quiz_id
                   WHERE s.status='pending' AND s.run_at <= NOW()
                   ORDER BY s.run_at FOR UPDATE SKIP LOCKED LIMIT $1""", limit)
            for row in rows:
                await conn.execute(
                    "UPDATE scheduled_quizzes SET status='running', started_at=NOW() WHERE id=$1",
                    row["id"])
            return rows

async def complete_schedule(schedule_id: int, error: Optional[str] = None):
    async with db_pool.acquire() as conn:
        await conn.execute(
            "UPDATE scheduled_quizzes SET status=$2, error=$3 WHERE id=$1",
            schedule_id, "failed" if error else "completed", error)

async def scheduled_quiz_worker(application):
    logger.info("📅 Scheduled quiz worker started")
    while True:
        try:
            rows = await claim_due_schedules(20)
            for row in rows:
                try:
                    ...
                    started = await begin_quiz(int(row["chat_id"]), fake_context)
                    if not started:
                        raise RuntimeError("Quiz could not be started (another quiz may already be active).")
                    await complete_schedule(int(row["id"]))
                except Exception as e:
                    logger.exception("Scheduled quiz %s failed", row["id"])
                    await complete_schedule(int(row["id"]), str(e)[:1000])
        except asyncio.CancelledError:
            break
        except Exception:
            logger.exception("Scheduled quiz worker error")
...
async def result_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db_pool.acquire() as conn:
        if quiz_id:
            row = await conn.fetchrow(
                """SELECT user_name, score, total_time, total_questions, correct, completed_at
                   FROM quiz_results WHERE quiz_id=$1 AND chat_id=$2 AND user_id=$3
                   ORDER BY completed_at DESC LIMIT 1""", quiz_id, chat_id, user_id)
        else:
            row = await conn.fetchrow(
                """SELECT r.user_name, r.score, r.total_time, r.total_questions, r.correct, r.completed_at, q.title
                   FROM quiz_results r LEFT JOIN quizzes q ON q.id=r.quiz_id
                   WHERE r.chat_id=$1 AND r.user_id=$2 ORDER BY r.completed_at DESC LIMIT 1""", chat_id, user_id)
    if not row:
        await update.message.reply_text("ℹ️ No saved result found for you yet.")
        return
    total = int(row["total_questions"] or 0)
    correct = int(row["correct"] if row["correct"] is not None else row["score"] or 0)
    accuracy = (correct / total * 100) if total else 0
    title = (row["title"] if "title" in row.keys() else None) or f"Quiz #{quiz_id or '-'}"
    await update.message.reply_text(
        f"📊 <b>Your Result</b>\n\n📌 {safe_html(title)}\n🎯 Score: <b>{row['score']}/{total or '?'}</b>\n✅ Correct: {correct}\n📈 Accuracy: {accuracy:.1f}%\n⏱️ Time: {format_time(float(row['total_time'] or 0))}",
        parse_mode="HTML")
...
async def leaderboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            """SELECT user_name, MAX(score) AS score, MIN(total_time) AS total_time
               FROM quiz_results WHERE chat_id=$1 GROUP BY user_id, user_name
               ORDER BY score DESC, total_time ASC LIMIT 20""", chat_id)
    if not rows:
        await update.message.reply_text("ℹ️ No leaderboard data yet.")
        return
    lines=["🏆 <b>Leaderboard</b>", "━━━━━━━━━━━━"]
    medals=["🥇","🥈","🥉"]
    for i,r in enumerate(rows):
        rank=medals[i] if i<3 else f"{i+1}."
        lines.append(f"{rank} {safe_html(r['user_name'] or 'User')} — <b>{int(r['score'] or 0)}</b> • {format_time(float(r['total_time'] or 0))}")
...
async def compare_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        try:
            quiz_id = int(context.args[0].replace("quiz_", ""))
        except ValueError:
            pass
    async with db_pool.acquire() as conn:
        if quiz_id:
            rows = await conn.fetch(
                """SELECT score,total_time,total_questions,correct,completed_at,quiz_id
                   FROM quiz_results WHERE chat_id=$1 AND user_id=$2 AND quiz_id=$3
                   ORDER BY completed_at DESC LIMIT 2""", chat_id, user_id, quiz_id)
        else:
            latest = await conn.fetchrow(
                """SELECT quiz_id FROM quiz_results
                   WHERE chat_id=$1 AND user_id=$2
                   ORDER BY completed_at DESC LIMIT 1""", chat_id, user_id)
            if not latest:
                rows = []
            else:
                ...
                rows = await conn.fetch(
                    """SELECT score,total_time,total_questions,correct,completed_at,quiz_id
                       ...
                       ORDER BY completed_at DESC LIMIT 2""", chat_id, user_id, quiz_id)
    if len(rows) < 2:
        await update.message.reply_text("ℹ️ At least two completed attempts of the same quiz are needed for comparison.")
        return
    latest, previous = rows[0], rows[1]
    score_delta = int(latest["score"] or 0) - int(previous["score"] or 0)
    time_delta = float(latest["total_time"] or 0) - float(previous["total_time"] or 0)
    latest_total = int(latest["total_questions"] or 0)
    previous_total = int(previous["total_questions"] or 0)
    latest_correct = int(latest["correct"] or 0)
    previous_correct = int(previous["correct"] or 0)
    latest_acc = (latest_correct / latest_total * 100) if latest_total else 0
    previous_acc = (previous_correct / previous_total * 100) if previous_total else 0
    arrow = "📈" if score_delta >= 0 else "📉"
    await update.message.reply_text(
        f"{arrow} <b>Result Comparison</b>\n\n"
        f"🎯 Score: {previous['score']} → <b>{latest['score']}</b> ({score_delta:+d})\n"
        f"✅ Accuracy: {previous_acc:.1f}% → <b>{latest_acc:.1f}%</b>\n"
        f"⏱️ Time: {format_time(float(previous['total_time'] or 0))} → <b>{format_time(float(latest['total_time'] or 0))}</b> ({time_delta:+.1f}s)\n"
        f"📅 Latest attempt: {latest['completed_at']}",
        parse_mode="HTML")

async def schedule_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args or len(context.args) < 3:
        await update.message.reply_text(
            "📅 <b>Schedule Quiz</b>\n\nUsage:\n<code>/sdl QUIZ_CODE YYYY-MM-DD HH:MM</code>\n\nRun this command in the target group as an admin.",
            parse_mode="HTML")
        return
    chat=update.effective_chat
    if chat.type not in ("group","supergroup"):
        await update.message.reply_text("❌ Run /sdl inside the group where the quiz should start.")
        return
    if not await is_target_chat_admin_or_owner(chat, update.effective_user.id):
        await update.message.reply_text("❌ Only a group admin/owner can schedule a quiz.")
        return
    code=context.args[0].replace("quiz_","")
    loaded=await load_quiz_by_code_or_id(code)
    if not loaded:
        await update.message.reply_text("❌ Quiz not found.")
        return
    owner_id,title,questions,short_code,quiz_id=loaded
    if int(owner_id)!=int(update.effective_user.id) and not await check_admin_or_owner(chat, update.effective_user.id, str(owner_id)):
        await update.message.reply_text("❌ Only the quiz owner or group owner/admin can schedule this quiz.")
        return
    try:
        local_tz = ZoneInfo(BOT_TIMEZONE)
        run_at = datetime.strptime(
            f"{context.args[1]} {context.args[2]}", "%Y-%m-%d %H:%M"
        ).replace(tzinfo=local_tz)
        run_at_utc = run_at.astimezone(timezone.utc)
    except (ValueError, KeyError):
        await update.message.reply_text(
            f"❌ Invalid date/time. Use YYYY-MM-DD HH:MM ({BOT_TIMEZONE})."
        )
        return
    if run_at_utc <= datetime.now(timezone.utc):
        await update.message.reply_text("❌ Schedule time must be in the future.")
        return
    sid=await create_schedule(update.effective_user.id, chat.id, quiz_id, run_at_utc)
    await update.message.reply_text(
        f"✅ <b>Quiz Scheduled</b>\n\n🆔 Schedule: <code>{sid}</code>\n📌 {safe_html(title)}\n📅 {run_at.strftime('%Y-%m-%d %H:%M')} ({BOT_TIMEZONE})\n📋 {len(questions)} questions",
        parse_mode="HTML")

async def schedules_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    rows=await list_schedules(update.effective_user.id, update.effective_chat.id if update.effective_chat.type in ("group","supergroup") else None)
    if not rows:
        await update.message.reply_text("📅 No pending scheduled quizzes.")
        return
    lines=["📅 <b>Pending Schedules</b>","━━━━━━━━━━━━"]
    for r in rows:
        lines.append(f"🆔 <code>{r['id']}</code> • {safe_html(r['title'])}\n📅 {r['run_at'].astimezone(ZoneInfo(BOT_TIMEZONE)).strftime('%Y-%m-%d %H:%M') + f' ({BOT_TIMEZONE})'}\n💬 <code>{r['chat_id']}</code>")
    await update.message.reply_text("\n".join(lines), parse_mode="HTML")

async def cancel_schedule_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usage: <code>/cancel_schedule SCHEDULE_ID</code>", parse_mode="HTML")
        return
    ok=await cancel_schedule(int(context.args[0]), update.effective_user.id)
    await update.message.reply_text("✅ Schedule cancelled." if ok else "❌ Schedule not found or already started.")

async def clone_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Usage: <code>/clone QUIZ_CODE</code>", parse_mode="HTML")
        return
    loaded=await load_quiz_by_code_or_id(context.args[0].replace("quiz_",""))
    if not loaded:
        await update.message.reply_text("❌ Quiz not found."); return
    _,title,questions,_,_=loaded
    saved=await save_quiz_async(update.effective_user.id, f"{title} (Copy)", deepcopy(questions))
    await update.message.reply_text(f"✅ Cloned quiz <b>{safe_html(title)}</b>\nNew code: <code>{saved[1]}</code>",parse_mode="HTML")

async def delete_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usage: <code>/delete QUIZ_ID</code>", parse_mode="HTML"); return
    ok=await delete_quiz_async(update.effective_user.id,int(context.args[0]))
    await update.message.reply_text("🗑️ Quiz deleted." if ok else "❌ Quiz not found or you are not the owner.")
...
async def help_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_text = (
        ...
        "• /skip - Skip current question (admin/owner)\n"
...
async def skip_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await check_admin_or_owner(update.effective_chat, update.effective_user.id, owner_id):
        await update.message.reply_text("❌ Only the quiz owner/admin can skip a question.")
        return
    await redis_client.set(f"quiz_skip:{chat_id}", "1", ex=30)
    try:
        await update.message.reply_text("⏭️ Current question will be skipped.")
    ...
    try:
        ...
        while index <= total_questions:
            ...
            while wait_time < timer:
                ...
                if await redis_client.get(f"quiz_skip:{chat_key}"):
                    await redis_client.delete(f"quiz_skip:{chat_key}")
                    try:
...
async def persist_results_to_db(
    ...
    try:
        ...
        total_questions = int(quiz_info.get("total_q") or 0)
        session_id = quiz_info.get("session_id")
            ...
            async with conn.transaction():
                for (
                    ...
                    await conn.execute(
                        ...
                        (quiz_id, chat_id, user_id, user_name, score, total_time, session_id, total_questions, correct)
...
async def post_init(
    ...
    scheduled_worker_task = asyncio.create_task(scheduled_quiz_worker(application))
    track_task(scheduled_worker_task)
...
def main():
    ...
    app.add_handler(CommandHandler(["sdl", "schedule"], schedule_command))
    app.add_handler(CommandHandler("schedules", schedules_command))
    app.add_handler(CommandHandler("cancel_schedule", cancel_schedule_command))
