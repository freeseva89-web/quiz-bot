        ...
        if edit_id:
            ...
            async with db_pool.acquire() as conn:
                await conn.execute(
                    ...
                    schedule_id, target_chat_id, control_message.message_id
                )
    except TelegramError:
        logger.exception("Failed to post schedule control card to target: %s", schedule_id)
...
async def edit_schedule_flow(query, context, schedule_id: int):
    # Control buttons are intentionally available on the target Group/Channel.
    async with db_pool.acquire() as conn:
