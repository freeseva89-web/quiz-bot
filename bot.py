...
async def post_init(
    ...
    # Webhook registration is handled by run_webhook() below.
    # Do not call set_webhook() here as well; doing both during startup can
    # trigger Telegram flood control when Render restarts/redeploys quickly.
...
def main():
    ...
    # This Render deployment is intentionally webhook-only.
    # Never fall back to polling: Telegram rejects getUpdates while a webhook is active.
    if not WEBHOOK_URL:
        raise RuntimeError(
