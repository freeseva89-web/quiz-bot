    ...
    if short_code:
        if live:
            ...
            rows.append([
                InlineKeyboardButton(
                    "🔒 Start this quiz",
                    callback_data=f"sched_locked_{schedule_id}"
                )
            ])
            rows.append([
                InlineKeyboardButton(
...
async def run_one_schedule(context, row):
    ...
    try:
        if target_type == "group":
            ...
            # Schedule Channel: activate the existing card only.
            # No automatic Share Polls/Share Quiz action and no card deletion.
            await update_schedule_status(schedule_id, "RUNNING")
            await delete_reminder(context, schedule_id)
...
async def handle_schedule_time(update, context, text: str):
    ...
    status_text = (
        "This quiz will start automatically at the scheduled time."
        if target_label == "Group"
        else "You will be able to start this quiz on the scheduled date and time."
    )
    ...
    try:
        ...
        logger.exception("Failed to create schedule card: %s", schedule_id)
...
async def cancel_schedule(query, context, schedule_id: int):
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT * FROM quiz_schedules WHERE id=$1",
            schedule_id
        )
    if not row:
    ...
    if not await schedule_user_authorized(query, context, row):
        await query.answer(
            "❌ Only the target Group/Channel Admin or Owner can cancel this schedule.",
            show_alert=True
        )
        return
