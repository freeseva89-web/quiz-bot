        ...
        try:
            async with db_pool.acquire() as conn:
                row = await conn.fetchrow(
                    """
                    INSERT INTO quizzes
                    (short_code, owner_id, title, questions, qshuffle, oshuffle, explanation, timer)
                    VALUES ($1, $2, $3, $4::jsonb, FALSE, FALSE, TRUE, 20)
    ...
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            (owner_id, quiz_id, target_chat_id, target_username, target_type, quiz_title,
             questions, qshuffle, oshuffle, explanation, timer, scheduled_at, status, reminder_sent, reminder_set)
            VALUES (
                $1,$2,$3,$4,
                CASE
                    ...
                    ELSE 'group'
                END,
                $6,$7::jsonb,$8,$9,$10,$11,$12,'pending',FALSE,FALSE
            )
            RETURNING id
            """,
            ...
            for row in reminder_rows:
                try:
                    ...
                    updated = await db_pool.execute(
                        """
                        UPDATE quiz_schedules
                        SET reminder_sent=TRUE, reminder_set=TRUE
                        WHERE id=$1
                          AND status IN ('pending','scheduled')
                          AND COALESCE(reminder_sent, reminder_set, FALSE)=FALSE
                        """, row["id"]
