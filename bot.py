...
async def execute_due_schedule(application: Application, schedule):
    ...
    try:
        ...
        logger.exception(
            "Telegram error while executing schedule %s",
            schedule_id
        )
        if schedule["target_type"] == "group":
            active = await redis_client.get(
                f"quiz_active:{target_chat_id}"
            )
            await mark_schedule_status(
                schedule_id,
                "LIVE" if active else "PENDING"
            )
            if active:
                await update_schedule_card(
                    application.bot,
        ...
        logger.exception(
            "Unexpected error while executing schedule %s",
            schedule_id
        )
        if schedule["target_type"] == "group":
            active = await redis_client.get(
...
async def schedule_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if action == "start":
        ...
        if result == "active":
            await query.answer(
                "ℹ️ This scheduled quiz is already running.",
                show_alert=True
            )
        elif result == "started":
            ...
            await query.answer(
                "⏳ Quiz is starting. Please try again in a moment.",
                show_alert=True
            )
        else:
            await query.answer(
