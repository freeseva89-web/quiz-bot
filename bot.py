    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("""
                ...
                ON scheduled_quizzes(target_chat_id, scheduled_at);

                ALTER TABLE scheduled_quizzes
                ADD COLUMN IF NOT EXISTS reminder_sent BOOLEAN NOT NULL DEFAULT FALSE;

                ALTER TABLE scheduled_quizzes
                ADD COLUMN IF NOT EXISTS execution_claimed BOOLEAN NOT NULL DEFAULT FALSE;
...
# ==========================================
# 14. GROUP SCHEDULE SYSTEM
# ==========================================
class _ScheduleContextProxy:
    """Minimal context adapter used by the persistent schedule worker."""
    def __init__(self, bot, user_data):
        self.bot = bot
        self.user_data = user_data


def schedule_now_ist() -> datetime:
    return datetime.now(SCHEDULE_TZ)


def format_schedule_time(value: datetime) -> str:
    return value.astimezone(SCHEDULE_TZ).strftime("%d %B %Y, %I:%M %p")
...
def parse_schedule_datetime(text: str) -> Optional[datetime]:
    ...
    for fmt in formats:
        try:
            return datetime.strptime(value, fmt).replace(tzinfo=SCHEDULE_TZ)
        except ValueError:
            continue
    return None


def build_schedule_keyboard(schedule_id: int, live: bool = False):
    start_text = "▶️ Start this quiz" if live else "🔒 Start this quiz"
    share_text = "📱 Share quiz" if live else "🔒 Share quiz"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(
            start_text,
...
def build_schedule_card_text(row, live: bool = False) -> str:
    scheduled_at = row["scheduled_at"] if isinstance(row, dict) else row.get("scheduled_at")
    title = row.get("title", "Quiz") if hasattr(row, "get") else "Quiz"
    if live:
        return (
            f"📅 Scheduled for: {format_schedule_time(scheduled_at)} IST\n\n"
            f"🟢 <b>LIVE</b>\n"
    ...
    async with db_pool.acquire() as conn:
        return await conn.fetchrow(
            ...
            INSERT INTO scheduled_quizzes
                (quiz_id, target_chat_id, scheduled_at, timezone,
                 status, created_by, quiz_settings)
            VALUES ($1, $2, $3, 'Asia/Kolkata', 'scheduled', $4, $5::jsonb)
            RETURNING id, quiz_id, target_chat_id, scheduled_at, timezone,
                      status, card_message_id, reminder_sent, execution_claimed,
                      quiz_settings, created_by
            """,
            quiz_id,
            target_chat_id,
...
async def get_schedule_async(schedule_id: int):
    async with db_pool.acquire() as conn:
        return await conn.fetchrow(
            """
            SELECT s.*, q.title, q.questions, q.short_code, q.owner_id
...
async def set_schedule_card_message_id_async(schedule_id: int, message_id: int) -> None:
    async with db_pool.acquire() as conn:
        await conn.execute(
            "UPDATE scheduled_quizzes SET card_message_id=$2 WHERE id=$1",
            schedule_id,
            message_id,
        )


async def claim_due_schedule_async(schedule_id: int):
    """Atomically transitions one schedule scheduled -> live."""
    async with db_pool.acquire() as conn:
        return await conn.fetchrow(
            """
            UPDATE scheduled_quizzes
            SET status='live', live_at=CURRENT_TIMESTAMP, execution_claimed=TRUE
            WHERE id=$1
...
async def claim_reminder_async(schedule_id: int) -> bool:
    async with db_pool.acquire() as conn:
        result = await conn.execute(
            """
            UPDATE scheduled_quizzes
...
async def cancel_schedule_async(schedule_id: int, user_id: int, chat_id: int):
    async with db_pool.acquire() as conn:
        return await conn.fetchrow(
            """
            UPDATE scheduled_quizzes s
            SET status='cancelled', cancelled_at=CURRENT_TIMESTAMP
            WHERE s.id=$1
              AND s.target_chat_id=$2
...
async def edit_schedule_card(context, row, live: bool):
    message_id = row.get("card_message_id")
    if not message_id:
        return
    text = build_schedule_card_text(row, live=live)
    ...
    for attempt in range(2):
        try:
            ...
            if attempt == 0:
                ...
                continue
            logger.warning("Schedule card flood limit persisted for schedule %s", row["id"])
            return
        except TelegramError as e:
            logger.warning("Schedule card edit failed for schedule %s: %s", row["id"], e)
            return


async def send_schedule_card(context, row):
    text = build_schedule_card_text(row, live=False)
    return await context.bot.send_message(
        chat_id=int(row["target_chat_id"]),
        text=text,
        parse_mode="HTML",
        reply_markup=build_schedule_keyboard(int(row["id"]), live=False),
    )


async def start_scheduled_quiz(context, row) -> bool:
    """Start the quiz using only data persisted with the schedule."""
    active = await redis_client.get(f"quiz_active:{row['target_chat_id']}")
    if active:
        return False

    loaded = await load_quiz_by_code_or_id(str(row["quiz_id"]))
    if not loaded:
        logger.error("Scheduled quiz %s no longer exists", row["quiz_id"])
        return False

    owner_id, title, questions, short_code, quiz_id = loaded
    settings = row.get("quiz_settings") or {}
    if isinstance(settings, str):
        settings = json.loads(settings)

    proxy = _ScheduleContextProxy(context.bot, {
        "owner_id": owner_id,
        "quiz_id": quiz_id,
        "short_code": short_code,
        "quiz_title": title,
    ...
    await begin_quiz(int(row["target_chat_id"]), proxy)
    return True


async def process_one_due_schedule(application: Application, schedule_id: int):
    claimed = await claim_due_schedule_async(schedule_id)
    if not claimed:
        return

    row = await get_schedule_async(schedule_id)
    if not row:
        return

    proxy = _ScheduleContextProxy(application.bot, {})
    # LIVE status and unlocked card are one execution flow. Telegram editing is
    # best-effort and never rolls the persistent schedule back to scheduled.
    await edit_schedule_card(proxy, row, live=True)
    try:
        started = await start_scheduled_quiz(proxy, row)
        if not started:
            logger.info("Schedule %s became LIVE but group already has an active quiz.", schedule_id)
    except Exception:
        logger.exception("Automatic scheduled quiz start failed for schedule %s", schedule_id)


async def process_schedule_reminder(application: Application, row):
    if not await claim_reminder_async(int(row["id"])):
        return
    try:
        await application.bot.send_message(
            ...
            text=(
                ...
                f"📅 Scheduled for: {format_schedule_time(row['scheduled_at'])} IST\n"
                "👥 This scheduled quiz will automatically start at the scheduled time."
            ),
            parse_mode="HTML",
        )
...
async def recover_and_run_schedules(application: Application):
    """Persistent DB-driven scheduler. Restarting the process never loses schedules."""
    logger.info("⏰ Loading persistent GROUP schedules from PostgreSQL...")
    try:
        async with db_pool.acquire() as conn:
        ...
        logger.info("⏰ Recovered %d pending group schedules.", len(pending))
    except Exception:
        logger.exception("Failed to load pending schedules during startup")
        ...
        try:
            ...
            async with db_pool.acquire() as conn:
                reminders = await conn.fetch(
                    """
                    SELECT id, scheduled_at, target_chat_id
                    FROM scheduled_quizzes
                    WHERE status='scheduled'
                      AND reminder_sent=FALSE
                      AND scheduled_at > CURRENT_TIMESTAMP
            ...
            for row in reminders:
                try:
                    full = await get_schedule_async(int(row["id"]))
                    if full:
                        await process_schedule_reminder(application, full)
                except Exception:
                    logger.exception("Reminder worker failed for schedule %s", row["id"])

            for row in due:
                try:
                    await process_one_due_schedule(application, int(row["id"]))
                except Exception:
                    logger.exception("Due schedule worker failed for schedule %s", row["id"])

            # Repair cards for schedules saved before a Telegram send/update completed.
            async with db_pool.acquire() as conn:
                missing_cards = await conn.fetch(
                    """
                    SELECT s.*, q.title
                    FROM scheduled_quizzes s
                    JOIN quizzes q ON q.id=s.quiz_id
                    WHERE s.status IN ('scheduled','live') AND s.card_message_id IS NULL
                ...
                try:
                    if row["status"] == "live":
                        msg = await application.bot.send_message(
                            chat_id=int(row["target_chat_id"]),
                            text=build_schedule_card_text(row, live=True),
                            parse_mode="HTML",
                            reply_markup=build_schedule_keyboard(int(row["id"]), live=True),
                        )
                    else:
                        msg = await send_schedule_card(application, row)
                    await set_schedule_card_message_id_async(int(row["id"]), int(msg.message_id))
                except TelegramError:
                    logger.warning("Could not repair schedule card %s", row["id"])

            await asyncio.sleep(SCHEDULE_POLL_INTERVAL)
        except asyncio.CancelledError:
            raise
        except Exception:
            logger.exception("Persistent schedule worker iteration failed; continuing")
            await asyncio.sleep(max(2.0, SCHEDULE_POLL_INTERVAL))


async def handle_schedule_start_callback(update, context):
    query = update.callback_query
    schedule_id = int(query.data.rsplit("_", 1)[1])
    row = await get_schedule_async(schedule_id)
    if not row or int(row["target_chat_id"]) != int(query.message.chat_id):
    ...
    if row["status"] == "scheduled":
        ...
        return
    if row["status"] != "live":
        await query.answer("❌ This schedule is not live.", show_alert=True)
        return

    await query.answer("▶️ Starting quiz...")
    proxy = _ScheduleContextProxy(context.bot, {})
    try:
        await start_scheduled_quiz(proxy, row)
    except Exception:
        logger.exception("Manual scheduled quiz start failed for %s", schedule_id)
        await query.message.reply_text("❌ Could not start the scheduled quiz.")


async def handle_schedule_share_callback(update, context):
    query = update.callback_query
    schedule_id = int(query.data.rsplit("_", 1)[1])
    row = await get_schedule_async(schedule_id)
    if not row or int(row["target_chat_id"]) != int(query.message.chat_id):
        ...
        return
    if row["status"] == "scheduled":
        await query.answer("🔒 Share is locked until the scheduled time.", show_alert=True)
        return
    if row["status"] != "live":
        await query.answer("❌ This schedule is not live.", show_alert=True)
        return

    await query.answer("📱 Share quiz link sent below.")
    short_code = row["short_code"]
    link = f"https://t.me/{BOT_USERNAME}?start=quiz_{short_code}"
    await query.message.reply_text(
        f"📱 <b>Share quiz</b>\n\n{safe_html(link)}",
        parse_mode="HTML",
...
async def handle_schedule_cancel_callback(update, context):
    ...
    if not await is_target_chat_admin_or_owner(query.message.chat, query.from_user.id):
        await query.answer("⚠️ Only Group Admin/Owner can cancel a schedule.", show_alert=True)
        return
    if row["status"] != "scheduled":
        await query.answer("❌ Only a scheduled item can be cancelled.", show_alert=True)
        return

    cancelled = await cancel_schedule_async(schedule_id, query.from_user.id, query.message.chat_id)
    if not cancelled:
    ...
    try:
        await query.edit_message_text(
            f"📅 Scheduled for: {format_schedule_time(row['scheduled_at'])} IST\n\n"
            "❌ <b>CANCELLED</b>\n"
            "👥 Scheduled quiz will not start automatically.",
            parse_mode="HTML",
        )
    except TelegramError:
        pass
...
async def button_handler(
    ...
    if data.startswith("schedule_start_"):
        await handle_schedule_start_callback(update, context)
        return

    if data.startswith("schedule_share_"):
        await handle_schedule_share_callback(update, context)
        return

    if data.startswith("schedule_cancel_"):
        await handle_schedule_cancel_callback(update, context)
        return
    ...
    # GROUP schedule creation only.
    if data == "schedule_group":
        if query.message.chat.type != "private":
            await query.answer("⏰ Schedule Group is available from private chat.", show_alert=True)
            return
        if not context.user_data.get("quiz_id") or not context.user_data.get("questions"):
            ...
            return
        if str(context.user_data.get("owner_id")) != str(query.from_user.id):
            await query.answer("⚠️ Only the quiz owner can create a schedule.", show_alert=True)
            return
        context.user_data["waiting_schedule_group"] = True
        context.user_data["waiting_schedule_datetime"] = False
        await query.answer()
        await query.message.reply_text(
            "👥 <b>Schedule Group Quiz</b>\n\n"
            "Send the Group username/link or Chat ID.\n"
            "Example: <code>-1001234567890</code> or <code>@mygroup</code>",
            parse_mode="HTML"
        )
        return
...
async def receive_text(
    ...
    # GROUP schedule target input. This flow is intentionally private-chat only.
    if context.user_data.get("waiting_schedule_group"):
        ...
        if chat.type not in ["group", "supergroup"]:
            await update.message.reply_text("❌ Schedule Group accepts only a Group/Supergroup.")
            return
        if not await is_target_chat_admin_or_owner(chat, update.effective_user.id):
            await update.message.reply_text("❌ Only the Group Admin or Owner can create a schedule for this group.")
            return
        context.user_data["schedule_target_chat_id"] = chat.id
        context.user_data["waiting_schedule_group"] = False
        context.user_data["waiting_schedule_datetime"] = True
        await update.message.reply_text(
            "📅 <b>Send scheduled date & time in IST</b>\n\n"
            "Format: <code>DD-MM-YYYY HH:MM</code>\n"
            "Example: <code>17-08-2026 10:30</code>\n\n"
            "Timezone is fixed to <b>Asia/Kolkata (IST)</b>.",
            parse_mode="HTML"
        )
        return

    if context.user_data.get("waiting_schedule_datetime"):
        scheduled_at = parse_schedule_datetime(text)
        if not scheduled_at:
            await update.message.reply_text("❌ Invalid date/time. Use <code>DD-MM-YYYY HH:MM</code> in IST.", parse_mode="HTML")
            return
        if scheduled_at <= schedule_now_ist() + timedelta(seconds=5):
            await update.message.reply_text("❌ Scheduled time must be in the future.")
            return

        target_chat_id = int(context.user_data.get("schedule_target_chat_id"))
        settings = {
        ...
        try:
            row = await create_schedule_async(
                int(context.user_data["quiz_id"]),
                target_chat_id,
                scheduled_at,
                update.effective_user.id,
                settings,
            )
            # DB is the source of truth. The startup worker repairs this card if Telegram fails here.
            msg = await send_schedule_card(context, row)
            await set_schedule_card_message_id_async(int(row["id"]), int(msg.message_id))
            await update.message.reply_text(
                f"✅ <b>Group schedule created.</b>\n\n"
                f"📅 {format_schedule_time(scheduled_at)} IST\n"
                f"👥 Target: <b>{safe_html(chat.title or str(chat.id))}</b>\n"
                "🔒 Buttons stay locked until the scheduled time.",
                parse_mode="HTML"
            )
        except Exception:
            logger.exception("Group schedule creation failed")
            await update.message.reply_text("❌ Failed to create the group schedule. The existing quiz remains unchanged.")
        finally:
            for key in ("waiting_schedule_group", "waiting_schedule_datetime", "schedule_target_chat_id"):
...
async def post_init(
    ...
    schedule_task = asyncio.create_task(
        recover_and_run_schedules(application)
    )
