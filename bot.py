...
# QUIZ SCHEDULER
# ==========================================
SCHEDULER_IST = __import__("zoneinfo").ZoneInfo("Asia/Kolkata")
SCHEDULER_UTC = __import__("zoneinfo").ZoneInfo("UTC")
SCHEDULER_POLL_SECONDS = 3
SCHEDULER_MIN_GAP_SECONDS = 300
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("""
                ...
                CREATE INDEX IF NOT EXISTS idx_scheduled_quizzes_chat_time
                ON scheduled_quizzes(chat_id, scheduled_at);

                CREATE INDEX IF NOT EXISTS idx_scheduled_quizzes_status_time
...
def build_settings_keyboard(data: dict) -> InlineKeyboardMarkup:
    ...
    # Schedule is additive and does not alter the existing quiz controls.
    if short_code:
        keyboard.append([
            InlineKeyboardButton(
                "🕐 Schedule Quiz",
...
async def button_handler(
    ...
    if data == "schedule_quiz":
        ...
        if not context.user_data.get("quiz_id") or not context.user_data.get("questions"):
            await query.answer("❌ Please load a quiz first.", show_alert=True)
            return
        context.user_data["scheduler_state"] = "GROUP"
        await query.answer()
        await query.message.reply_text(
    ...
    if data.startswith("schedule_cancel:"):
        try:
            ...
            return
        ok, msg = await scheduler_cancel(query.get_bot(), sid, query.from_user.id)
        await query.answer(msg, show_alert=True)
        if ok:
            await scheduler_update_card(
                query.get_bot(), sid,
...
async def receive_text(
    ...
    # Scheduler input is isolated from all existing quiz-creation states.
    scheduler_state = context.user_data.get("scheduler_state")
    if scheduler_state:
        text = update.message.text.strip()

        if scheduler_state == "GROUP":
            chat, err = await scheduler_resolve_group(context.bot, text)
            if err:
                await update.message.reply_text(err)
                return
            if not await scheduler_user_is_admin(
                context.bot, chat.id, update.effective_user.id
            ):
                await update.message.reply_text(
                    "❌ You must be a group administrator or owner to schedule a quiz."
                )
                return
            context.user_data["scheduler_chat_id"] = chat.id
            context.user_data["scheduler_chat_title"] = getattr(chat, "title", "")
            context.user_data["scheduler_state"] = "DATE"
            await update.message.reply_text(
                "✅ <b>Group verification successful.</b>\n\n"
                "📅 <b>Send the quiz date in YYYY-MM-DD format.</b>\n"
                "Example: <code>2026-08-25</code>",
                parse_mode="HTML"
            )
            return

        if scheduler_state == "DATE":
            try:
                datetime.strptime(text, "%Y-%m-%d")
            except ValueError:
                await update.message.reply_text("❌ Invalid date. Please use YYYY-MM-DD.")
                return
            context.user_data["scheduler_date"] = text
            context.user_data["scheduler_state"] = "TIME"
            await update.message.reply_text(
                "⏰ <b>Send the quiz time in 24-hour HH:MM format.</b>\n"
        ...
        if scheduler_state == "TIME":
            scheduled = scheduler_parse_datetime(
                context.user_data["scheduler_date"], text
            )
            if not scheduled:
                await update.message.reply_text(
                    "❌ Invalid or past time. Please send HH:MM, for example 20:00."
                )
                return
            chat = SimpleNamespace(
                id=context.user_data["scheduler_chat_id"],
                title=context.user_data.get("scheduler_chat_title")
            )
            sid, err = await scheduler_create(
            ...
            if err:
                await update.message.reply_text(err)
                return
            await scheduler_post_card(context.bot, sid)
            for key in (
                "scheduler_state",
                "scheduler_chat_id",
                "scheduler_chat_title",
                "scheduler_date"
            ):
...
# ==========================================
# 19A. QUIZ SCHEDULER (ADDITIVE)
# ==========================================
from types import SimpleNamespace


def scheduler_parse_target(raw: str):
    raw = (raw or "").strip()
    if re.fullmatch(r"-100\d+", raw) or re.fullmatch(r"-\d+", raw):
        return int(raw)
    m = re.fullmatch(r"https?://t\.me/([A-Za-z0-9_]{5,})/?", raw)
    if m:
        return "@" + m.group(1)
...
async def scheduler_resolve_group(bot, raw):
    target = scheduler_parse_target(raw)
    if target is None:
        return None, "❌ Invalid group identifier. Send a public @username, public Telegram link, or Chat ID."
    try:
        chat = await bot.get_chat(target)
    except TelegramError:
        return None, "❌ I could not find that group. Please check the username, link, or Chat ID."
    if chat.type not in ("group", "supergroup"):
        return None, "❌ The target must be a group or supergroup."
    try:
        me = await bot.get_me()
        member = await bot.get_chat_member(chat.id, me.id)
        if member.status not in ("administrator", "creator"):
            return None, "❌ The bot must be an administrator in that group."
    except TelegramError:
        return None, "❌ I could not verify the bot's administrator status in that group."
    return chat, None


async def scheduler_user_is_admin(bot, chat_id, user_id):
    try:
...
def scheduler_parse_datetime(date_text, time_text):
    try:
        dt = datetime.strptime(
            f"{date_text.strip()} {time_text.strip()}",
            "%Y-%m-%d %H:%M"
        ).replace(tzinfo=SCHEDULER_IST)
    except ValueError:
        return None
    if dt <= datetime.now(SCHEDULER_IST):
        return None
    return dt


async def scheduler_create(quiz_id, created_by, chat, scheduled_ist, quiz_config):
    scheduled_utc = scheduled_ist.astimezone(SCHEDULER_UTC)
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            await conn.execute(
                "SELECT pg_advisory_xact_lock($1)",
                abs(int(chat.id)) % 2147483647
            )
            conflict = await conn.fetchval(
                """
                SELECT EXISTS(
                    SELECT 1 FROM scheduled_quizzes
                    WHERE chat_id=$1
                      AND status IN ('PENDING','WAITING','RUNNING')
                      AND ABS(EXTRACT(EPOCH FROM (scheduled_at - $2))) < 300
                )
                """,
                chat.id, scheduled_utc
            )
            if conflict:
                return None, "❌ Another quiz is already scheduled within 5 minutes of that time. Please choose another time."

            row = await conn.fetchrow(
                """
                INSERT INTO scheduled_quizzes
                (quiz_id, created_by, quiz_config, chat_id, chat_title, scheduled_at)
                VALUES ($1,$2,$3::jsonb,$4,$5,$6)
                RETURNING id
                """,
                quiz_id, created_by, json.dumps(quiz_config or {}, ensure_ascii=False),
                chat.id, getattr(chat, "title", None), scheduled_utc
            )
            return int(row["id"]), None


async def scheduler_get(schedule_id):
    return await db_pool.fetchrow(
        "SELECT * FROM scheduled_quizzes WHERE id=$1", schedule_id
    )


async def scheduler_post_card(bot, schedule_id):
    row = await scheduler_get(schedule_id)
    if not row:
        return
    quiz = await db_pool.fetchrow(
        "SELECT title FROM quizzes WHERE id=$1", row["quiz_id"]
    )
    if not quiz:
        return

    when = row["scheduled_at"].astimezone(SCHEDULER_IST)
    msg = await bot.send_message(
        row["chat_id"],
        f"📚 <b>{safe_html(quiz['title'])}</b>\n\n"
        f"📅 <b>Date:</b> {when.strftime('%d %B %Y')}\n"
        f"⏰ <b>Time:</b> {when.strftime('%I:%M %p')} IST\n\n"
        "This quiz will automatically start at the scheduled time.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton(
                "❌ Cancel Schedule",
                callback_data=f"schedule_cancel:{schedule_id}"
            )
        ]])
    )
...
async def scheduler_update_card(bot, schedule_id, status_text, cancellable=False):
    row = await scheduler_get(schedule_id)
    if not row or not row["schedule_message_id"]:
        return
    quiz = await db_pool.fetchrow(
        "SELECT title FROM quizzes WHERE id=$1", row["quiz_id"]
    )
    ...
    try:
        await bot.edit_message_text(
            ...
            f"{status_text}",
            chat_id=row["chat_id"],
            message_id=row["schedule_message_id"],
            parse_mode="HTML",
            reply_markup=markup
        )
    except TelegramError:
...
async def scheduler_cancel(bot, schedule_id, user_id):
    row = await scheduler_get(schedule_id)
    if not row:
        return False, "❌ Schedule not found."
    if row["status"] in ("COMPLETED", "CANCELLED", "MISSED"):
        return False, "❌ This schedule is no longer active."
    if not await scheduler_user_is_admin(bot, row["chat_id"], user_id):
        return False, "❌ Only a group administrator or owner can cancel this schedule."
...
async def scheduler_start_quiz(application, row):
    quiz = await db_pool.fetchrow(
        "SELECT owner_id, title, questions, id FROM quizzes WHERE id=$1",
        row["quiz_id"]
    )
    if not quiz:
        await db_pool.execute(
            "UPDATE scheduled_quizzes SET status='MISSED', completed_at=NOW() WHERE id=$1",
            row["id"]
        )
        return
    ...
    # Existing begin_quiz() is unchanged. This lightweight context provides
    # only the attributes that the existing engine already uses.
    saved_config = row["quiz_config"] or {}
    ...
    scheduled_context = SimpleNamespace(
        ...
        user_data={
            ...
            "qshuffle": bool(saved_config.get("qshuffle", False)),
            "oshuffle": bool(saved_config.get("oshuffle", False)),
            "explanation": bool(saved_config.get("explanation", True)),
            "timer": int(saved_config.get("timer", 20))
        }
    )
    await begin_quiz(row["chat_id"], scheduled_context)


async def scheduler_tick(application):
    now = datetime.now(SCHEDULER_UTC)

    # A RUNNING schedule is complete when its existing quiz engine has cleared
    # quiz_active. This also makes the next waiting schedule eligible.
    running = await db_pool.fetch(
        "SELECT id, chat_id FROM scheduled_quizzes WHERE status='RUNNING'"
    )
    for row in running:
        if not await redis_client.get(f"quiz_active:{row['chat_id']}"):
            await db_pool.execute(
                """
                UPDATE scheduled_quizzes
            ...
            await scheduler_update_card(
                application.bot, row["id"],
                "✅ <b>This scheduled quiz has completed.</b>"
            )
    ...
    for row in rows:
        sid = row["id"]
        seconds = (row["scheduled_at"] - now).total_seconds()

        if row["status"] == "PENDING":
            if 0 < seconds <= 600 and not row["reminder_10_sent"]:
                try:
                    await application.bot.send_message(
                        row["chat_id"],
                        "All students get ready, the quiz will start in 10 minutes."
                    )
                    await db_pool.execute(
            ...
            if 0 < seconds <= 300 and not row["reminder_5_sent"]:
                try:
                    await application.bot.send_message(
                        row["chat_id"],
        ...
        active = await redis_client.get(f"quiz_active:{row['chat_id']}")
        if active:
            if row["status"] == "PENDING":
                await db_pool.execute(
                    "UPDATE scheduled_quizzes SET status='WAITING' WHERE id=$1",
                    sid
                )
                await scheduler_update_card(
                    application.bot, sid,
                    "⏳ <b>This quiz is waiting for the previous quiz to finish.</b>",
                    cancellable=True
                )
            continue

        # If another earlier scheduled quiz for this group is due, let it go first.
        earlier = await db_pool.fetchval(
            """
            SELECT EXISTS(
                SELECT 1 FROM scheduled_quizzes
                WHERE chat_id=$1
            ...
            """,
            row["chat_id"], row["scheduled_at"], sid
        )
        if earlier:
            continue

        changed = await db_pool.execute(
            """
            UPDATE scheduled_quizzes
        ...
        await scheduler_update_card(
            application.bot, sid,
            "🚀 <b>This quiz is starting now.</b>"
        )
        try:
            await scheduler_start_quiz(application, row)
        except Exception:
            logger.exception("Scheduled quiz %s failed to start.", sid)
            await db_pool.execute(
                "UPDATE scheduled_quizzes SET status='MISSED', completed_at=NOW() WHERE id=$1",
                sid
            )
            try:
                await application.bot.send_message(
                    row["chat_id"],
                    "❌ The scheduled quiz could not be started."
                )
            except TelegramError:
                pass
...
async def scheduler_loop(application):
    ...
    while True:
        try:
            await scheduler_tick(application)
        except asyncio.CancelledError:
            raise
        except Exception:
