...
async def claim_due_schedule(schedule_id: int):
    if not db_pool:
        return None
    async with db_pool.acquire() as conn:
        return await conn.fetchrow(
            ...
            WHERE id=$1 AND status IN ('PENDING','WAITING') AND scheduled_at <= CURRENT_TIMESTAMP
            RETURNING *
            """,
            schedule_id,
        )
...
async def execute_due_schedule(application: Application, schedule):
    ...
    if schedule["target_type"] == "channel":
        # Channel schedules only make the existing card live; no quiz is posted automatically.
        await mark_schedule_status(schedule_id, "LIVE")
        if schedule.get("reminder_message_id"):
            await delete_schedule_message(application.bot, target_chat_id, schedule["reminder_message_id"])
        await update_schedule_card(application.bot, schedule_id, live=True)
        logger.info("Channel schedule %s is now LIVE", schedule_id)
        return

    # Group schedules reuse the exact existing Live Quiz Engine.
    ...
    result = await begin_quiz(target_chat_id, schedule_context, schedule_id=schedule_id)
    if result == "active":
        await mark_schedule_status(schedule_id, "WAITING")
        await update_schedule_card(application.bot, schedule_id, live=False)
        return
    if result not in ("started", "locked"):
        await mark_schedule_status(schedule_id, "FAILED")
        await update_schedule_card(application.bot, schedule_id, live=False)
        return
    if schedule["reminder_message_id"]:
        await delete_schedule_message(application.bot, target_chat_id, schedule["reminder_message_id"])
    await update_schedule_card(application.bot, schedule_id, live=True)
    logger.info("Group schedule %s started existing Live Quiz Engine", schedule_id)
    ...
    while True:
        try:
            ...
            for running in running_rows:
                ...
                elif not await redis_client.get(f"quiz_active:{target_id}"):
            ...
            for due in due_rows:
                claimed = await claim_due_schedule(int(due["id"]))
                if claimed:
...
async def schedule_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if action == "start":
        if row["status"] not in ("LIVE", "RUNNING"):
            await query.answer("🔒 This quiz is locked until the scheduled time.", show_alert=True)
            return True
        # Group scheduled quizzes keep the existing admin/owner permission model.
        # Channel LIVE cards are intentionally usable by members, matching the
        # requested Channel behavior: once LIVE, Start/Share are unlocked.
        if row["target_type"] != "channel":
            if not await verify_schedule_actor(row, query.from_user.id, context.bot):
                await query.answer("⚠️ Only the target Admin/Owner can start this scheduled quiz.", show_alert=True)
                return True
        quiz = await load_schedule_quiz(row)
        if not quiz:
        ...
        if result == "active":
            await query.answer("⚠️ A quiz is already running in this chat.", show_alert=True)
        elif result == "started":
